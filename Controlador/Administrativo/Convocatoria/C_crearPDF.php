...
require('../../../Modelo/Administrativo/Acceso_Datos.php');//Se incluye la Libreria de Acceso a la Base de Datos.
include_once("../../../Modelo/Administrativo/conexionreporte.php");
conectar_bd();
...
$pdf->Ln();
$pdf->Ln();
$respuesta = mysql_query('SELECT VC_Nombre_Entidad FROM TB_PROPUESTA_Organizacion_Precontractual WHERE PK_Id_Organizacion=1;');
$pdf->SetWidths(array($width));
		while($fila = mysql_fetch_array($respuesta)){
			$pdf->Row(array('PROPUESTA PRESENTADA POR LA ORGANIZACION '.strtoupper($fila["VC_Nombre_Entidad"])));
		}
...
$respuesta = mysql_query('SELECT * FROM TB_PROPUESTA_Proyecto WHERE PK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFont('Arial','B',9);
$pdf->SetFillColor(0,69,134);
$pdf->SetTextColor(255,255,255);
...
$respuesta = mysql_query('SELECT VC_Nombre_Entidad, VC_Nit, VC_Direccion, LOC.VC_Nom_Localidad, VC_Objeto_Social, VC_Telefono, VC_Correo, IN_Estrato_Sede FROM TB_PROPUESTA_Organizacion_Precontractual ORG JOIN tb_localidades LOC ON ORG.IN_Id_Localidad = LOC.PK_Id_Localidad WHERE PK_Id_Organizacion=1;');
$pdf->SetFillColor(204,204,204);
$pdf->Ln();
...
$respuesta = mysql_query('SELECT VC_Representante_Legal, VC_Cedula_RL, VC_Direccion_RL, VC_Telefono_RL, VC_Correo_RL FROM TB_PROPUESTA_Organizacion_Precontractual WHERE PK_Id_Organizacion=1;');
$pdf->SetFillColor(204,204,204);
...
$respuesta = mysql_query('SELECT VC_Nombre_Director, VC_Cedula_Director, VC_Direccion_Director, VC_Telefono_Director, VC_Correo_Director FROM TB_PROPUESTA_Proyecto WHERE PK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFillColor(204,204,204);
...
$pdf->Ln();
$pdf->SetTextColor(0,0,0);
$respuesta = mysql_query('SELECT VC_Antecedentes FROM TB_PROPUESTA_Proyecto WHERE PK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFont('Arial','',9);
		$width=190;
		$pdf->SetWidths(array($width));
		while($fila = mysql_fetch_array($respuesta)){
...
$respuesta = mysql_query('SELECT * FROM TB_PROPUESTA_Historial_Organizacion WHERE FK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFillColor(204,204,204);
$pdf->SetTextColor(0,0,0);
...
$pdf->SetFont('Arial','B',9);
		...
		while($fila = mysql_fetch_array($respuesta)){
			...
			$dias = $interval->format('%m meses %d dias');

			//$dias	= (date('d/m/Y', strtotime($fila['VC_Inicio'])))-(date('d/m/Y', strtotime($fila['VC_Fin'])))/86400;
			//$dias 	= abs($dias); $dias = floor($dias);
			//$meses = $dias/30;
			$pdf->Row(array($fila['VC_Entidad'],$fila['VC_Nombre_Proyecto'],$dias,$fila["VC_Actividad_Desarrollada"],$fila["FL_Cantidad_Beneficiados"]));
		}
		$pdf->Ln();
...
$pdf->Ln();
$pdf->SetTextColor(0,0,0);
$respuesta = mysql_query('SELECT VC_Descripcion_Problema FROM TB_PROPUESTA_Proyecto WHERE PK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFont('Arial','',9);
		$width=190;
		$pdf->SetFont('Arial','',9);
		$pdf->SetWidths(array($width));
...
$pdf->Ln();
$respuesta = mysql_query('SELECT VC_Objetivo_General FROM TB_PROPUESTA_Proyecto WHERE PK_Id_Proyecto="'.$id_proyecto.'";');
		$width=190;
		$pdf->SetWidths(array($width));
		while($fila = mysql_fetch_array($respuesta)){
...
$pdf->Ln();
		...
		$pdf->Ln();
$respuesta = mysql_query('SELECT * FROM TB_PROPUESTA_Proyecto_Obj_Especificos WHERE FK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFont('Arial','',9);
		$width=190/2;
...
$pdf->Ln();
$pdf->SetTextColor(0,0,0);
$respuesta = mysql_query('SELECT VC_Descripcion_Proyecto FROM TB_PROPUESTA_Proyecto WHERE PK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFont('Arial','',9);
		$width=190;
		$pdf->SetWidths(array($width));
		while($fila = mysql_fetch_array($respuesta)){
...
$pdf->Ln();
$pdf->SetTextColor(0,0,0);
$respuesta = mysql_query('SELECT VC_Beneficiarios, FL_Cantidad_Beneficiados FROM TB_PROPUESTA_Proyecto WHERE PK_Id_Proyecto="'.$id_proyecto.'";');
		$pdf->SetFont('Arial','',9);
		$width=190;
		$pdf->SetWidths(array($width));
		...
		while($fila = mysql_fetch_array($respuesta)){
			...
			$pdf->MultiCell(190, 5,'Se registraron '.$fila['FL_Cantidad_Beneficiados'].' beneficiarios los cuales estan clasificados o distribuidos mediante la siguiente tabla', 1, 'J', false);
			$pdf->Ln();
				...
				$pdf->MultiCell(190/3, 5,utf8_decode('Adulto Mayor (60 años y más): ').$categorias[7], 1, 'J', false);
				$pdf->SetFont('Arial','B',9);$pdf->setX(75);					
				...
				$pdf->MultiCell(190/3, 5,utf8_decode('Medios comunitarios: ').$categorias[16], 1, 'J', false);
				$pdf->SetFont('Arial','B',9);$pdf->setX(75);					
...
$width=190/5;
		...
		$pdf->Ln();
$respuesta = mysql_query('SELECT * FROM TB_PROPUESTA_Proyecto_Metas WHERE FK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFont('Arial','',9);
		$width=190/5;
...
$width=190/5;
		...
		$pdf->Cell($width,8,'Periodo',1,0,'C'); //Titulo del reporte
		$pdf->Ln();
$respuesta = mysql_query('SELECT * FROM TB_PROPUESTA_Proyecto_Indicadores WHERE FK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFont('Arial','',9);
		$width=190/5;
...
$pdf->SetTextColor(0,0,0);
$width=190/3;
		$pdf->Cell($width,8,utf8_decode('Nombre'),1,0,'C'); //Titulo del reporte
		$pdf->Cell($width+($width/2),8,utf8_decode('Perfil'),1,0,'C'); //Titulo del reporte
		$pdf->Cell($width/2,8,utf8_decode('Rol'),1,0,'C'); //Titulo del reporte
		$pdf->Ln();
$respuesta = mysql_query('SELECT * FROM TB_PROPUESTA_Proyecto_Equipo WHERE FK_Id_Proyecto="'.$id_proyecto.'";');
$pdf->SetFont('Arial','',9);
		$width=190/3;
		$pdf->SetWidths(array($width,$width+($width/2),$width/2));
		while($fila = mysql_fetch_array($respuesta)){
...
$pdf->SetFillColor(204,204,204);
		...
		$pdf->Ln();
$respuesta = mysql_query('SELECT * FROM TB_PROPUESTA_Proyecto_Archivos WHERE FK_Id_Usuario="'.$id_proyecto.'";');
$pdf->SetFont('Arial','',9);
		$width=190/2;
		$pdf->SetWidths(array($width));
		while($fila = mysql_fetch_array($respuesta)){
