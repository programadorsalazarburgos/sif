...
$pdf->MultiCell(190,8,utf8_decode('REPORTE DE ENTREGA DE SUBSANACIONES'),1,'C', true); //Titulo del reporte
...
$pdf->SetWidths(array($width));
		while($fila = mysql_fetch_array($respuesta)){
			$pdf->Row(array(utf8_decode('DOCUMENTOS DE SUBSANACIÓN ENTREGADOS POR LA ORGANIZACIÓN '.strtoupper($fila["VC_Organizacion"]))));
		}

$pdf->SetFont('Arial','',9);
$pdf->Ln();
//DOCUMENTOS HABILITANTES SUBIDOS POR EL USUARIO.
$respuesta = mysql_query('SELECT * FROM tb_propuesta_proyecto_archivos WHERE FK_Id_Usuario="'.$_SESSION['id_usuario'].'" AND VC_Fuente="SUBSANACION";');
$pdf->SetFont('Arial','B',9);
$pdf->SetTextColor(0,0,0);
$pdf->SetFillColor(204,204,204);
$pdf->Ln();
$pdf->Cell(190,8,utf8_decode('DOCUMENTOS DE SUBSANACIÓN ENTREGADOS'),0,0,'C', true); //Titulo del reporte
$pdf->Ln();
$pdf->SetFont('Arial','B',9);
		$width=190/2;
		...
		$pdf->Ln();
$pdf->Output('ReporteSubsanaciones.pdf','I');
?>