...
if(isset($_FILES))
{
	$nombre_carpeta = "habilitantes/".$_SESSION["nombre_usuario"]."2016-12";//."-".$fecha;
	...
	if(!is_dir($nombre_carpeta)){
		...
		mysqli_query(ConexionDatos::conexion(),'DELETE FROM TB_PROPUESTA_Proyecto_Archivos WHERE FK_Id_Usuario = "'.$_SESSION['id_usuario'].'" AND VC_Fuente="HABILITANTE";');
	...
	foreach($_FILES as $file)
	{
		...
		if(move_uploaded_file($file['tmp_name'], $uploaddir.$Nombre_Archivo))
		{
			...
			mysqli_query(ConexionDatos::conexion(),"INSERT INTO TB_PROPUESTA_Proyecto_Archivos (FK_Id_Usuario, VC_Nombre_Archivo, VC_Ubicacion, VC_Tipo, VC_Fuente) VALUES ('$Id_Usuario','$Nombre_Archivo','$Ubicacion_Archivo','$Tipo_Archivo', 'HABILITANTE')");
			mysqli_close(ConexionDatos::conexion());
		}
		else
		{
	...
	mysqli_query(ConexionDatos::conexion(),'UPDATE TB_PROPUESTA_Usuarios SET IN_Bandera_Habilitantes=1 WHERE PK_Id_Usuario = "'.$_SESSION['id_usuario'].'";');
	mysqli_close(ConexionDatos::conexion());
}
else
{
