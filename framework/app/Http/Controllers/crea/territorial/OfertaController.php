...
namespace App\Http\Controllers\crea\territorial;
...
use App\Models\crea\territorial\Preinscripcion;
use App\Models\crea\territorial\EmprendeEstudiante;
use App\Models\crea\territorial\Estudiante;
use App\Models\crea\territorial\EstudianteDetalleAnio;
use App\Models\crea\territorial\EstudianteArchivo;
...
class OfertaController extends Controller
{
	...
	public function getOptionsOfertaDisponible(Request $request){
		...
		$grupos = Emprende::select("PK_Grupo", "FK_clan", "FK_area_artistica", "FK_modalidad", "IN_modalidad_atencion", "TX_observaciones", "IN_cupos")->where([
			["abierto_publico", 1],
			["estado", 1]
		])
		->with("areaArtistica:FK_Value,VC_Descripcion")
		->with("horarios")
		->with("modalidad:PK_Id_Modalidad,VC_Nom_Modalidad")
		...
		->havingRaw("IN_cupos - estudiantes_activos_count > 0")
		->orderBy("FK_clan", "ASC")
		->get();
	...
	public function getGrupos(Request $request){
		$grupos = Emprende::select("PK_Grupo as text", "PK_Grupo as value")->where([
			["abierto_publico", 1],
			["estado", 1],
	...
	public function getCuposGrupo(Request $request){
		$cupos = Emprende::select("IN_cupos")->where("PK_Grupo", $request->id_grupo)
	...
	public function aprobarPreinscripcion(Request $request){
		...
		try {
			...
			$emprende_estudiante = new EmprendeEstudiante;
	...
	public function getDatosCrea($datos, $id_grupo){
		...
		$datos_crea = Emprende::select("FK_clan")
