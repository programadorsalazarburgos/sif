    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="panel-heading">
                        <div class="page-header">
                            <h5 style="text-align: center;">REPORTE DE ASISTENCIA MENSUAL CREA</h5>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <div class="container-fluid">
                        <form action="" method="post" @submit.prevent="consulta" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="col-md-2">
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label>Mes</label>
                                                <div class="custom-file">
                                                    <select class="form-control" v-model="mes" required @input="getListadoConvenios">
                                                        <option value="0" disabled>Seleccione</option>
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                ...
                                                <div class="custom-file">
                                                    <v-select :options="arrayConvenios" v-model="convenio" label="text" @input="getListadoColegios"></v-select>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label>Colegio</label>
                                                <div class="custom-file">
                                                    <v-select :options="arrayColegios" v-model="colegio" label="text"></v-select>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <button type="submit" v-on:click="tipoReporteConsulta()" class="btn-success">
                                Consultar <i class="fa fa-search-plus"></i>
                            </button>
                            <button type="submit" v-on:click="tipoReporteDescarga()" class="btn-primary">
                ...
                <div class="card-body">
                    <h4 class="card-title"><label class="badge badge-success">REPORTE MENSUAL</label></h4>
                    <div class="table-responsive">
                        <table class="table dataTable no-footer" id="sampleTable">
                            <thead>
                                <tr>
                                    <th class="sorting" tabindex="0" aria-controls="table_listado_informes" rowspan="1" colspan="1" aria-label="Grupo: activate to sort column ascending" style="width: 89.7869px;">Grupo</th>
                                    <th class="sorting" tabindex="0" aria-controls="table_listado_informes" rowspan="1" colspan="1" aria-label="Artista: activate to sort column ascending" style="width: 418.267px;">Artista</th>
                                    <th class="sorting" tabindex="0" aria-controls="table_listado_informes" rowspan="1" colspan="1" aria-label="Fecha de Registro/Edición: activate to sort column ascending" style="width: 303.537px;">Fecha de Registro/Edición</th>
                                    <th class="sorting" tabindex="0" aria-controls="table_listado_informes" rowspan="1" colspan="1" aria-label="Fecha revisión: activate to sort column ascending" style="width: 198.381px;">Fecha revisión</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="data in arrayReporte" :key="data.id" role="row" class="odd">
                                    <td>{{data.nombre_grupo}}</td>
                                    <td>{{data.nombre_artista}}</td>
                                    <td>{{data.DT_fecha_creacion}}</td>
                                    <td>{{data.DT_fecha_creacion}}</td>
                                </tr>
                            </tbody>
                        </table>
...
export default {
    ...
    mounted() {
        ...
        this.getListadoLineasAtencion();

    },
    methods: {
        generarReportePDF(d, id_informe, firma_escaneada) {
            let suplencia = false;
            ...
            var horizontal = this.generarReportePDFHorizontal(d);
            ...
            var dd = {
                ...
                footer: function (currentPage, pageCount) {
                    return 'Reporte mensual ' + d.datos_basicos.mes_reporte + ' de asistencias grupo ' + d.datos_basicos.nombre_grupo + ' del artista formador: ' + d['datos_basicos']['nombre_artista'] + ' página ' + currentPage.toString() + ' de ' + pageCount;
                },
            };
            pdfMake.createPdf(dd).download("Reporte_mensual.pdf");
        },
        generarReportePDFHorizontal(d) {
            let suplencia = false;
            d.detallado_sesiones_clase.forEach(sesion => {
                if (sesion.suplencia == 1) {
                    suplencia = true;
        ...
        async tipoReporteDescarga2() {
            var url_ok_obj = '../../src/ArtistaFormador/ReporteMensualController/';
            
            for (var i = 0; i < this.miArray.length; i += 1) {
                var id_informe = this.miArray[i];
                ...
                var url = 'consultarJSONReporte?id_informe=' + id_informe;
                console.log(this.miArray[i]);
                axios({
                    url: url,
                    method: 'POST',
                    data: datos,
                   ...
                   // console.log(id_informe, "informe");
                    var firma = response.data[0]['TX_Firma_Escaneada'];
                    if (firma != null) {
                        var data = response.data[0]['TX_json'];
                        data = $.parseJSON(data);
                        for (var i = 0; i < this.miArray.length; i += 1) {
                           // this.generarReportePDF(data, miArray[i], firma);
                        }
                    }
                });
            }
        ...
        consulta() {
            let me = this;
            if (me.tipoReporte == 1) {
                var url = 'getDataReporte?lineaAtencion=' + me.linea +
                    '&anio=' + me.anio.VC_Descripcion +
                    '&mes=' + me.mes +
                    '&colegioId=' + me.colegio.value +
                    '&colegioNombre=' + me.colegio.text +
                    '&convenio=' + me.convenio.value +
                    '&tipoReporte=' + me.tipoReporte;
                axios.get(url).then(function (response) {
                        var respuesta = response.data;
                        me.arrayReporte = respuesta.data;
                        me.tabla();
            ...
            if (me.tipoReporte == 2) {
                me.mensaje();
                var url = 'getDataReporte_digital?lineaAtencion=' + me.linea +
                    '&anio=' + me.anio.VC_Descripcion +
                    '&mes=' + me.mes +
                    '&colegioId=' + me.colegio.value +
                    '&colegioNombre=' + me.colegio.text +
                ...
                axios({
                    ...
                    if (response.data === "") {
                        ...
                        var urlZIP = getUrl.protocol + '//' + getUrl.host + '/sif/framework/';
                        var fileLink = document.createElement('a');
                        fileLink.href = urlZIP + me.colegio.text + '.zip';
                        fileLink.setAttribute('download', me.colegio.text + '.zip');
                        document.body.appendChild(fileLink);
                        fileLink.click();
                        me.eliminarZip(me.colegio.text + '.zip');
                    }
                });
            }
        },
        eliminarZip(colegio) {
            let me = this;
            var url = 'deleteColegioZip?colegio=' + colegio;
            axios({
                url: url,
                method: 'POST',
            }).then((response) => {
        ...
        getListadoAnios() {
            axios
                ...
                .then(response => {
                    var respuesta = response.data;
                    this.arrayAnios = respuesta;
                })
                .catch(error => {});
        },

        getListadoLineasAtencion() {
            axios
                ...
                .then(response => {
                    var respuesta = response.data;
                    this.arrayLineas = respuesta;
                })
                .catch(error => {});
        },
        getListadoConvenios() {
            let me = this;
            var url = 'getConvenios?lineaAtencion=' + me.linea + '&anio=' + me.anio.VC_Descripcion;
            axios.get(url).then(function (response) {
                    var respuesta = response.data;
                    me.arrayConvenios = respuesta;
                })
                .catch(function (error) {
                    var response = error.response.data;
                });
        ...
        getListadoColegios() {
            let me = this;
            var url = 'getColegios?convenio=' + me.convenio.value + '&anio=' + me.anio.VC_Descripcion;
            axios.get(url).then(function (response) {
                    var respuesta = response.data;
                    me.arrayColegios = respuesta;
                })
                .catch(function (error) {
                    var response = error.response.data;
                });
