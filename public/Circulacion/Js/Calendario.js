var url_ok_obj = '../../src/Circulacion/CirculacionController/';
$(function(){

	var table_estudiantes = $("#table_estudiantes").DataTable({
		iDisplayLength: '10',
		"language": {
			"lengthMenu": "Ver _MENU_ registros por pagina",
			"zeroRecords": "No hay información, lo sentimos.",
			"info": "Mostrando pagina _PAGE_ de _PAGES_",
			"infoEmpty": "No hay registros disponibles",
			"infoFiltered": "(filtered from _MAX_ total records)",
			"search": "Filtrar"
		},
		dom: 'Bfrtip',
		buttons: [
		{
			extend: 'excel',
			title: 'Estudiantes del evento'
		}
		]
	});

	$("#SL_crea").html(parent.getOptionsClanes()).selectpicker('refresh').selectpicker('selectAll');
	$("#SL_area_artistica").html(parent.getOptionsAreasArtisticas()).selectpicker('refresh').selectpicker('selectAll');

	$("#SL_crea").change(function(ev){
		cargarEventosMes(mes,anio);
	});
	$("#SL_area_artistica").change(function(ev){
		cargarEventosMes(mes,anio);
	});

	$(".cambia_mes").click(function(){
		var periodo = $(this).data("fecha").split("-");
		var mes = periodo[1];
		var anio = periodo[0];
		//cargarEventosMes(mes,anio);console.log(periodo);
		location.href="Calendario.php?mes="+periodo[1]+"&anio="+periodo[0];
	});
	$(".new_evento").click(function(){
		var fecha = $(this).data("fecha");
		$("#fecha_nuevo_evento").html("Agregar evento para el: " + fecha);
		$("#fecha_new_evento").val(fecha);
		$("#modal-nuevo-evento").modal("show");
	});
	$("td").delegate(".ver_evento","click",function(){
		var id_evento = $(this).data("id-evento");
		$("#modal-detalle-evento").modal("show");
		$("#modal-detalle-evento").data("id_evento",id_evento);
		$("#nav_detalle_evento").trigger("click");

	});

	$("#tab_estudiantes_cupos").delegate(".permiso_padres","change",function(ev){
		var estado_permiso = ($(this).prop('checked')?1:0);
		datos = {
			p1: {
				id_evento : $(this).data("id_evento"),
				id_estudiante : $(this).data("id_estudiante"),
				id_usuario: parent.idUsuario,
				permiso: estado_permiso
			}
		};
		$.ajax({
			url:url_ok_obj+'actualizarPermisoPadres',
			type:'POST',
			data: datos,
		}).done(function(data){
			console.log(data);
		}).fail(function(jqXHR,textStatus){
			parent.mostrarAlerta('error','Error','No se ha podido cambiar el estado del permiso de padres del beneficiario al evento por:' + textStatus);
		});
	});

	$("#BT_descargar_pdf").click(function(){
		generarReporteEvento($("#modal-detalle-evento").data("id_evento"));
	});
	$("#nav_detalle_evento").click(function(){
		datos = {
			'p1' : {
				'id_evento' : $("#modal-detalle-evento").data("id_evento")
			}
		}
		$.ajax({
			url: url_ok_obj+'calendarioConsultarDetallesEvento',
			type: 'POST',
			data: datos,
			async: false
		}).done(function(result){
		      $("#div_detalle_evento").html(result);
		}).fail(function(result){
		      console.log("Error: "+result);
		});
		
	});
	$("#nav_estudiantes_cupos").click(mostrarInformacionEstudiantesEvento);

	function mostrarInformacionEstudiantesEvento(){
		datos = {
			'p1' : {
				'id_evento' : $("#modal-detalle-evento").data("id_evento"),
				'opcion' : {
					'id_rol' : parent.idRol
				}
			}
		}
		$.ajax({
			url: url_ok_obj+'cargarEstudiantesDeEventoDistrital',
			type: 'POST',
			data: datos,
			async: false
		}).done(function(data){
			var data_estudiantes =  "<table class='table table-striped table-bordered table-hover' id='table_estudiantes'><thead><tr><th>Permiso Padres</th><th>Identificación</th><th>Nombre</th><th>Opciones</th></tr></thead><tbody>";
			data_estudiantes += data;
			data_estudiantes += "</tbody></table>"
		      $("#div_table_estudiantes").html(data_estudiantes);
		}).fail(function(result){
		      console.log("Error: "+result);
		});
		var table_estudiantes = $("#table_estudiantes").DataTable({ 
			"pageLength": 10,
			"language": {
				"lengthMenu": "Ver _MENU_ registros por pagina",
				"zeroRecords": "No hay información, lo sentimos.",
				"info": "Mostrando pagina _PAGE_ de _PAGES_",
				"infoEmpty": "No hay registros disponibles",
				"infoFiltered": "(filtered from _MAX_ total records)",
				"search": "Filtrar"
			},
			dom: 'Bfrtip',
			buttons: [
			{
				extend: 'excel',
				exportOptions: {
					columns: ':visible' 
				},
				title: 'Estudiantes evento'
			}
			]
		});
		table_estudiantes.on( 'draw', function () {
			$('.permiso_padres').bootstrapToggle({
				on: 'SÍ',
				off: 'NO',
				onstyle: 'success',
				offstyle: 'danger'
			});
		}).draw();
    	table_estudiantes.search('').draw();
	}
});

function cargarEventosMes(mes, anio){
	var datos = {
		'p1' : {
			'mes' : mes,
			'anio' : anio,
			'id_crea' : $("#SL_crea").val().join(),
			'id_area_artistica' : $("#SL_area_artistica").val().join()
		}
	};
	$.ajax({
		url:url_ok_obj+'calendarioConsultarEventoMesAnio',
		type:'POST',
		data: datos
	}).done(function(data){
		$(".ver_evento").remove();
		$(".salto_linea").remove();
		try{
			data = $.parseJSON(data);
			var icono = [];
			icono["MU"]="picture";
			icono["VI"]="camera";
			icono["FE"]="music";
			icono["CA"]="road";
			$.each(data,function(i,item){
				var tipo_evento = item.VC_Nombre.substring(0,2);
				var fecha = item.DT_Fecha_Inicio.split("-");
				$("#dia-"+fecha[2].substring(0,2)).append(" <br class='salto_linea'><a class='ver_evento' data-id-evento='" + item.PK_Id_Evento + "' href='#' data-toggle='tooltip' title='" + item.VC_Nombre + "'><span class='glyphicon glyphicon-" + icono[tipo_evento] + "'>  " + tipo_evento + "-" + item.PK_Id_Evento + "</span></a>");
			});
		}catch(ex){
			console.log(ex);
			console.log(data);
		}
	});
}

function consultarDatosEventoParaReporte(id_evento){
	var dato_return = [];
	var datos = {
		'p1' : {
			'id_evento' : id_evento,
		}
	};
	$.ajax({
		url:url_ok_obj+'consultarDatosBasicosEvento',
		type:'POST',
		data: datos,
		async: false,
	}).done(function(data){
		try{
			data = $.parseJSON(data);
			dato_return[0] = data[0];
		}catch(ex){
			console.log(ex);
			console.log(data);
		}
	});

	$.ajax({
		url:url_ok_obj+'consultarDatosMultiplesEvento',
		type:'POST',
		data: datos,
		async : false,
	}).done(function(data){
		try{
			data = $.parseJSON(data);
			dato_return[1] = data;
		}catch(ex){
			console.log(ex);
			console.log(data);
		}
	});
	return dato_return;
}

function generarReporteEvento(id_evento){
	var dato_evento = consultarDatosEventoParaReporte(id_evento);
	if(dato_evento[0]['estado_evento'] == 1){
		parent.mostrarAlerta("warning","Evento continua activo","Contactese con el administrador del evento para que lo finalice y así se pueda generar el reporte respectivo.");
	}else{
		var ul_crea = [];
		var ul_area_artistica = [];
		var ul_artista = [];
		$.each(dato_evento[1]['crea'],function(key,value){
			ul_crea.push(value['VC_Nom_Clan']);
		});
		$.each(dato_evento[1]['area_artistica'],function(key,value){
			ul_area_artistica.push(value['VC_Nom_Area']);
		});
		$.each(dato_evento[1]['artista_formador'],function(key,value){
			ul_artista.push(value['VC_Primer_Apellido'] + ' ' + value['VC_Segundo_Apellido'] + ' ' + value['VC_Primer_Nombre'] + ' ' + value['VC_Segundo_Nombre']);
		});
		total_asistencia_evento = Number.parseInt(dato_evento[0]['IN_asistencia_nna']);
		total_asistencia_evento += Number.parseInt(dato_evento[0]['IN_asistencia_padres']);
		total_asistencia_evento += Number.parseInt(dato_evento[0]['IN_asistencia_publico']);
		console.log(dato_evento);
		var contenido_reporte = {
			pageSize: 'legal',
			pageOrientation: 'portrait',
			// pageMargins:[19,55,20,70],
			//watermark: {text: 'NO VALIDO', color: 'grey', opacity: 0.333, bold: true, italics: false,marginX:-5,marginY:60},
			content : [
			{
				style: 'table_encabezado',
				table: {
					widths: [ '20%', '60%', '20%' ],
					body: [
					[
					{
						rowSpan: 3,
						image: 'data:image/png;base64,',
						height:80,
						width:90,
						alignment: 'center'
					},
					{
						text: 'IDARTES - CREA',
						alignment: 'center',
						bold: true,
						fontSize: 17

					},
					{
						rowSpan: 3,
						image: 'data:image/png;base64,',
						height:80,
						width:90,
						alignment: 'center'
					}
					],
					[
					'',
					{
						text: 'CIRCULACIÓN',
						alignment: 'center',
						bold: true,
						fontSize: 15

					},
					'',
					],
					[
					'',
					{
						text: 'INFORME ESTADISTICO DE EVENTO',
						alignment: 'center',
						bold: true,
						fontSize: 15

					},
					'',
					]
					]
				}
			},
			{
				text : '\n Estos son los datos del evento ' + id_evento +', registrado en el Sistema Integrado de Formación (SIF) de la subdirección de formación artística generado desde el SIF a las ' + parent.getFechaYHoraServidor(),

			},
			{
				style: 'table_datos_basicos',
				fontSize: 11,
				alignment: 'left',
				table:{
					widths: [ '20%', '80%'],
					body: [
					[
					{
						colSpan: 2,
						alignment: 'center',
						fontSize: 13,
						text: 'DATOS BÁSICOS',
						bold: true,
					},
					''
					],
					[
					{
						text: 'Responsable:',
						bold: true,
					},
					{
						text: dato_evento[0]['organizador'],

					}
					],
					[
					{
						text: 'Duración (Fecha):',
						bold: true,
					},
					{
						text: 'Del ' + dato_evento[0]['DT_Fecha_Inicio'] + ' al ' + dato_evento[0]['DT_Fecha_Fin'],

					}
					],
					[
					{
						text: 'Nombre:',
						bold: true,
					},
					{
						text: dato_evento[0]['VC_Nombre'] != null ? dato_evento[0]['VC_Nombre']:"",

					}
					],
					[
					{
						text: 'Lugar:',
						bold: true,
					},
					{
						text: dato_evento[0]['VC_Lugar'] != null ? dato_evento[0]['VC_Lugar']:"",

					}
					],
					[
					{
						text: 'Objetivo:',
						bold: true,
					},
					{
						text: dato_evento[0]['VC_Objetivo'] != null ? dato_evento[0]['VC_Objetivo']:"",

					}
					],
					[
					{
						text: 'Observaciones:',
						bold: true,
					},
					{
						text: dato_evento[0]['VC_Observaciones'] != null ? dato_evento[0]['VC_Observaciones']:"",

					}
					],
					]
				}
			},
			{
				text: ' '
			},
			{
				style: 'table_datos_mutliples',
				fontSize: 11,
				alignment: 'left',
				table:{
					widths: [ '20%', '40%','40%'],
					body: [
					[
					{
						colSpan: 3,
						alignment: 'center',
						fontSize: 13,
						text: 'DATOS MULTIPLES O DETALLADOS',
						bold: true,
					},
					'',
					''
					],
					[
					{
						text: 'CREA(S):',
						bold: true,
					},
					{
						colSpan: 2,
						ul: ul_crea,

					},
					''
					],
					[
					{
						text: 'ÁREAS ARTÍSTICAS:',
						bold: true,
					},
					{
						colSpan: 2,
						ul: ul_area_artistica,

					},
					''
					],
					[
					{
						text: 'ARTISTAS FORMADORES:',
						bold: true,
					},
					{
						colSpan: 2,
						ul: ul_artista,

					},
					''
					],
					[
					{
						rowSpan: 4,
						text: 'AFORO:',
						bold: true,
					},
					{
						text: 'NNA:'

					},
					{
						alignment: 'right',
						text: dato_evento[0]['IN_asistencia_nna']
					}
					],
					[
					'',
					{
						text: 'PADRES:'

					},
					{
						alignment: 'right',
						text: dato_evento[0]['IN_asistencia_padres']
					}
					],
					[
					'',
					{
						text: 'PÚBLICO:'

					},
					{
						alignment: 'right',
						text: dato_evento[0]['IN_asistencia_publico']
					}
					],

					[
					'',
					{
						bold: true,
						text: 'TOTAL ASISTENTES:'

					},
					{
						alignment: 'right',
						bold: true,
						fontSize: 15,
						text: total_asistencia_evento,
					}
					],
					[
					{
						text: 'PUBLICO ESPERADO SEGÚN PLANEACIÓN DEL EVENTO:',
						bold: true,
						colSpan: 2,
					},
					'',
					{
						alignment: 'right',
						bold: true,
						fontSize: 12,
						text: dato_evento[0]['IN_publico_asistente'],

					},
					],
					]
				}
			},
			{
				text: '\nEste reporte se encuentra en etapa de desarrollo y no es valido aún como documentación oficial del Sistema Integrado de Formación (SIF)'
			}
			]
		};
		pdfMake.createPdf(contenido_reporte).open();
	}
}