...
$(function(){
	...
	$("#SL_grupo").html(parent.consultarGruposActivosActualmente()).selectpicker("refresh").trigger("change");
	$("#TX_mes").datepicker({
		format: "yyyy-mm",
		language: 'es',
	...
	}).datepicker("setDate", new Date(anio_mes_dia));

	$("#BT_generar_reporte").click(function(ev){
		var datos = {
			'p1': {
		...
		$.ajax({
			...
			// url:url_ok_obj+'consultarDatosReporteUnicoCreaEnCasa',
			type:'POST',
			data: datos,
			async: true,
			dataType: 'json'
			...
			parent.mostrarAlerta("error","Error","No se ha podido consultar el reporte consolidado del grupo");
		    console.log(result);
		});
		
	});

	function obtenerSitioAtencionGrupoLaboratorio(id_grupo){
		var sitio_atencion = 'NO';
		var datos = {
			'p1': {
				'id_grupo' : id_grupo
			}
		...
		$.ajax({
			url:url_ok_obj+'obtenerSitioAtencionGrupoLaboratorio',
			type:'POST',
			data: datos,
			async: false,
		}).done(function(data){
			console.log(data);
			 try{
				...
				console.log(ex);
				parent.mostrarAlerta("error","Fallo decodificar JSON","El sistema no ha podido decodificar los datos de lugar de atención y horario, por favor intente nuevamente.");
			}
		    sitio_atencion = data;
		}).fail(function(result){
			parent.mostrarAlerta("error","Error","No se ha podido consultar el sitio de atención del grupo");
		    console.log(result);
		    sitio_atencion = "Error consultando el sitio de atención";
		});
		return sitio_atencion;
	}

	function cargarGruposAsistencia(id_usuario){
		datos = {
			'p1': {
				'mes': $("#TX_mes").val(),
				'id_usuario': $("#SL_artista_formador").val()
			}
		};
		$.ajax({
			url: '../../src/ArtistaFormador/ReporteMensualController/consultarGruposRegistroMiAsistenciaMes',
			type: 'POST',
			data: datos,
			async: false
		}).done(function(data){
	...
	function generarReportePDF(d){
		var widthsDataSesion = [ '30%', '10%'];
		var bodyDataSesion = [];

		var institucion_atencion_nombre = 'Institución Educativa:';
		var institucion_atencion = d['datos_basicos']['nombre_colegio'];
		if (d.datos_basicos.tipo_grupo == 'laboratorio_clan'){
			var dias_semana = ["","Lunes","Martes","Miercoles","Jueves","Viernes","Sabado","Domingo"];
			var sitio_atencion_y_horario = obtenerSitioAtencionGrupoLaboratorio(d.datos_basicos.id_grupo);
			institucion_atencion_nombre = 'Centro de atención - horario:';
			institucion_atencion = sitio_atencion_y_horario.sitio_atencion;
			institucion_atencion += " -> ";
			sitio_atencion_y_horario.horario_grupo.forEach(function(df){
				institucion_atencion += dias_semana[df.IN_dia] += "(" + df.TI_hora_inicio_clase + " a " + df.TI_hora_fin_clase + ")";
			});
		}
		...
		try{
			...
			lp.forEach(function(l){
				...
				dt.forEach(function(df){
					...
					if(as[df.fecha_clase][l.id] == null){
						estado_asistencia = 'NR'
					}else{
		...
		var dd = {
			// watermark: {text: 'APROBADO', color: 'grey', opacity: 0.333, bold: true, italics: false,marginX:-5,marginY:60},
			// watermark: {text: 'creaencasa.idartes.gov.co', color: 'gray', opacity: 0.8, bold: true, italics: false,marginX:0,marginY:60,font: 'ArialOl'},
			...
			content : [
			{
				style: 'tableExample',
			        table: { // Encabezado del reporte.
			        	...
			        	{
			        		rowSpan: 2,
			        		text: 'REPORTE DE ASISTENCIA MENSUAL POR GRUPO - CREA',
			        		alignment: 'center',
			        		bold: true,
			        		fontSize: 15
			    ...
			    },
			    {
			        ...
			        table: { // Datos del artista
			        	...
			        	{
			        		text: ' Equipo CREA EN CASA', //d['datos_basicos']['nombre_artista'],
			    ...
			    },
			    {
			    	...
			    	table: {
			    		...
			    		[
			    		{
			    			text: institucion_atencion_nombre,
			    			bold: true,
			    		},
			    		{
			    			colSpan: 5,
			    			text:  institucion_atencion,
			...
			pdfMake.createPdf(dd).download("ReporteUnicoAsistenciaGrupoCreaEnCasa.pdf");
