var url_ok_obj = '../../../src/PedagogicoNidos/RegistroFortalecimientoController/'; 
var url_asis = '../../../src/Beneficiarios/Controlador/RegistroAsistenciaController.php';

$(function() {

  $("#SL_Mes_Consolidado").html(parent.getOptionsParametroDetalle(8)).selectpicker("refresh");

    var id_usuario = $("#id_usuario").val();

    $("#modal_nombre_evento_LAB").delegate("input[type=text].fecha", "focusin", function(){
      $(this).datepicker({
        format: 'yyyy-mm-dd',
        language: 'es',
        autoclose: true,
        endDate: '+0d',
        startDate: '2019-01-01'
      });
    });

      $("#modal_nombre_evento").delegate("input[type=text].fecha", "focusin", function(){
      $(this).datepicker({
        format: 'yyyy-mm-dd',
        language: 'es',
        autoclose: true,
        endDate: '+0d',
        startDate: '2019-01-01'
      });
    });

    $("#SL_Localidad").html(parent.getOptionsLocalidad()).selectpicker("refresh");
    $("#SL_Localidad").html(parent.getOptionsLocalidad()).change(function() {
      $("#SL_Lugar").html(getOptionsLugaresLocalidad($("#SL_Localidad").val())).selectpicker("refresh");
    }).selectpicker("refresh");

    $("#SL_Lugar").html(getOptionsLugaresLocalidad()).change(function() {
      $("#TX_Evento").html(getOptionsEventosLugar($("#SL_Lugar").val())).selectpicker("refresh");
    }).selectpicker("refresh");


   /* ---------- LABORATORIO ---------- */
   $("#SL_Localidad_LAB").html(parent.getOptionsLocalidad()).selectpicker("refresh");
   $("#SL_Localidad_LAB").html(parent.getOptionsLocalidad()).change(function() {
      $("#SL_Lugar_LAB").html(getOptionsLugaresLocalidad($("#SL_Localidad_LAB").val())).selectpicker("refresh");
    }).selectpicker("refresh");
    $("#SL_Lugar_LAB").html(getOptionsLugaresLocalidad()).change(function() {
     $("#TX_Evento_LAB").html(getOptionsEventosLugar($("#SL_Lugar_LAB").val())).selectpicker("refresh");
    }).selectpicker("refresh");   


getTipoPersona();

InfoNombreEaat();
getArtistasTerritorio();
getArtistasLaboratorio();

getEquipoCentral();


/* $("#SL_Tipo_Persona").click(function(){
     $("#div_equipo_central1").show("refresh");
    getEquipoCentral();
}); */


$(".mayuscula").keyup(function() {
  $(this).val($(this).val().toUpperCase());
});


$("#TX_Evento").on("change", function(){
InfoHoraEvento();
});

$("#TX_Evento_LAB").on("change", function(){
InfoHoraEvento_LAB();
});

$("#nav_consultar_fortalecimiento").click(function() {
$("#SL_Mes_Consolidado").change(cargarFortalecimientosMes);
});

function cargarFortalecimientosMes() {
  $("#div_fortalecimientos_mes").show("refresh");
  ConsultarFortalecimientosEaat();
}

$("table").delegate(".Consultar-Experiencia", "click", function() {
    var idFortalecimiento = $(this).data("idexperiencia");
    procesarInfoPDFInforme(idFortalecimiento);
  });



$("#TX_Evento").on("change", function(){
  if($("#TX_Evento").val() == 1){
      $("#modal_nombre_evento").modal("show");
      $("#div-nuevo-evento").empty();
      $("#div-nuevo-evento").append("Ingrese el nombre del evento<br><input type='text' class='form-control evento' id='TX_Nombre_Evento' name='nombre_evento'<br>");
      $("#div-nuevo-evento").append("<span id='error-evento'></span>");
      $("#div-nuevo-evento").append("<br>Seleccione la fecha<br> <input type='text' class='form-control fecha' id='TX_Fecha_Encuentro' readonly  name='TX_Fecha_Encuentro' placeholder='Fecha de la experiencia'>");
      $("#div-nuevo-evento").append("<span id='error-fecha'></span>")
  }
});

$("#TX_Evento_LAB").on("change", function(){
  if($("#TX_Evento_LAB").val() == 1){
      $("#modal_nombre_evento_LAB").modal("show");
      $("#div-nuevo-evento_LAB").empty();
      $("#div-nuevo-evento_LAB").append("Ingrese el nombre del evento<br><input type='text' class='form-control evento' id='TX_Nombre_Evento_LAB' name='nombre_evento_LAB'<br>");
      $("#div-nuevo-evento_LAB").append("<span id='error-evento'></span>");
      $("#div-nuevo-evento_LAB").append("<br>Seleccione la fecha<br> <input type='text' class='form-control fecha' id='TX_Fecha_Encuentro_LAB' readonly  name='TX_Fecha_Encuentro_LAB' placeholder='Fecha de la experiencia'>");
      $("#div-nuevo-evento_LAB").append("<span id='error-fecha'></span>")
  }
});


$("#btn-guardar-observacion").click(function(){
  id_lugar = $("#SL_Lugar").val();
  if($("#TX_Nombre_Evento").val() == ""){
    $("#error-evento").empty();
    $("#error-evento").css("color", "red");
    $("#error-evento").text("Por favor escriba el nombre del evento");
  }
  if($("#TX_Fecha_Encuentro").val() == ""){
    $("#error-fecha").empty();
    $("#error-fecha").css("color", "red");
    $("#error-fecha").text("Por favor seleccione la fecha del evento");
  }else{
  $("#modal_nombre_evento").modal("hide");
  guardarFormularioCreacion();
  }
});


$("#btn-guardar-observacion_LAB").click(function(){
  id_lugar_LAB = $("#SL_Lugar_LAB").val();
  if($("#TX_Nombre_Evento_LAB").val() == ""){
    $("#error-evento").empty();
    $("#error-evento").css("color", "red");
    $("#error-evento").text("Por favor escriba el nombre del evento");
  }
  if($("#TX_Fecha_Encuentro_LAB").val() == ""){
    $("#error-fecha").empty();
    $("#error-fecha").css("color", "red");
    $("#error-fecha").text("Por favor seleccione la fecha del evento");
  }else{
  $("#modal_nombre_evento_LAB").modal("hide");
  guardarFormularioCreacion_LAB();
  }
});



/*-----------------Función para guardar los datos del nuevo grupo-----------------------*/
function guardarFormularioCreacion() {
  datos = {
    p1: {
      'localidad': $("#SL_Localidad").val(),
      'lugar': $("#SL_Lugar").val(),
      'evento': $("#TX_Nombre_Evento").val(),
      'fecha': $("#TX_Fecha_Encuentro").val(),
      'eaat': parent.idUsuario
        }
  };
  $.ajax({
     url: url_ok_obj+"crearNuevoEvento",
    type: 'POST',
    data: datos,
    async: false,
    success: function(data) {
      $("#TX_Evento").html(getOptionsEventosLugar(id_lugar)).selectpicker("refresh");
      parent.mostrarAlerta("success","Operación exitosa","Evento creado correctamente, por favor selecciónelo de la lista.");
      //limpiarFormulario();
    }
  });
}


/*-----------------Función para guardar los datos del nuevo grupo-----------------------*/
function guardarFormularioCreacion_LAB() {
  datos = {
     p1: {
      'localidad': $("#SL_Localidad_LAB").val(),
      'lugar': $("#SL_Lugar_LAB").val(),
      'evento': $("#TX_Nombre_Evento_LAB").val(),
      'fecha': $("#TX_Fecha_Encuentro_LAB").val(),
      'eaat': parent.idUsuario
        }
  };
  $.ajax({
    url: url_ok_obj+"crearNuevoEvento",
    type: 'POST',
    data: datos,
    async: false,
    success: function(data) {
      $("#TX_Evento_LAB").html(getOptionsEventosLugar(id_lugar_LAB)).selectpicker("refresh");
      parent.mostrarAlerta("success","Operación exitosa","Evento creado correctamente, por favor selecciónelo de la lista.");
      //limpiarFormulario();
    }
  });
}
function InfoNombreEaat() {
  datos = {
    'p1': parent.idUsuario
  };
  $.ajax({
    url: url_ok_obj+"getNombreEaat",
    type: 'POST',
    data: datos,
    async: false,
    success: function(data){
      datos = $.parseJSON(data);
      $(".TX_Eaat").text(datos['NOMBRE']);
      $("#TX_Territorio").text(datos['TERRITORIO']);
      $("#TX_Nom_Laboratorio").text(datos['LABORATORIO']);
      Laboratorio = datos['ID_LABORATORIO'];
      Territorio = datos['IDTERRITORIO'];
    },

  });
}



function getTipoPersona() {
  var datos = {
    funcion: 'getTipoPersona',
    'p1': parent.idUsuario
   };
    $.ajax({
    url: url_asis,
    type: 'POST',
    data: datos,
    success: function(data) {
     datos = $.parseJSON(data);
     tipo_persona = datos['FK_Tipo_Persona'];
     console.log(tipo_persona);
    if(tipo_persona == 22){
      $("#div_fortalecimientos_eaat").show("refresh");

        /*getArtistasDupla(parent.idUsuario);
        $("#li-registrar-sistematizacion").show();
        $("#li-editar-sistematizacion").show();
        $(".nav #li-registrar-sistematizacion").addClass('active');
        $("#registrar-sistematizacion").addClass('active');*/
    }
    if(tipo_persona == 25){
      $("#div_fortalecimientos_coordinador").show("refresh");
      $("#div_equipo_central1").show("refresh");

      

       /* $("#li-revision-sistematizacion").show();
        $(".nav #li-revision-sistematizacion").addClass('active');
        $("#revision-sistematizacion").addClass('active');
        $("#SL_Dupla").html(getDuplas()).selectpicker("refresh"); */
    }
  },
  async: false
    });
}



function InfoHoraEvento() {
  datos = {
    'funcion': '',
    'p1': $("#TX_Evento").val()
  };
  $.ajax({
    url: url_ok_obj+"getHoraEvento",
    type: 'POST',
    data: datos,
    async: false,
    success: function(data){
      datos = $.parseJSON(data);
      $("#TX_Fecha_Encuentro1").val(datos['FECHA']);
    },
  });
}

function InfoHoraEvento_LAB() {
  datos = {
    'p1': $("#TX_Evento_LAB").val()
  };
  $.ajax({
    url: url_ok_obj+"getHoraEvento",
    type: 'POST',
    data: datos,
    async: false,
    success: function(data){
      datos = $.parseJSON(data);
      $("#TX_Fecha_Encuentro1_LAB").val(datos['FECHA']);
    },
  });
}


/*-----------------Función para mostrar los lugares de la localidad -----------------------*/
function getOptionsLugaresLocalidad(parametro) {
  var mostrar = "";
  var datos = {
    'p1': parametro
  };
  $.ajax({
    url: url_ok_obj+"getOptionsLugaresLocalidad",
    type: 'POST',
    data: datos,
    success: function(data) {
      mostrar += data
    },
    async: false
  });
  return mostrar;
}

/*-----------------Función para mostrar los eventos del lugar -----------------------*/
function getOptionsEventosLugar(parametro) {
  var mostrar = "";
  var datos = {
    'p1': parametro
  };
  $.ajax({
    url: url_ok_obj+"getOptionsEventosLugar",
    type: 'POST',
    data: datos,
    success: function(data) {
      mostrar += data
    },
    async: false
  });
  return mostrar;
}

/************** ENCABEZADO REPORTE PDF ********************/
function cargarEncabezadoReporte(idFortalecimiento) {
  var mostrar;
  var datos = {
    'p1': idFortalecimiento
  };
  $.ajax({
    url: url_ok_obj+"consultarEncabezadoFortalecimiento",
    type: 'POST',
    data: datos,
    dataType: 'json',
    success: function(data) {
      mostrar = data[0];
    },
    async: false
  });
  return mostrar;
}
/********** ARTISTAS FORTALECIMIENTO REPORTE PDF ********************/
function cargarArtistasFortalecimiento(idFortalecimiento) {
  var mostrar;
  var datos = {
    'p1': idFortalecimiento
  };
  $.ajax({
    url: url_ok_obj+"consultarArtistasFortalecimiento",
    type: 'POST',
    data: datos,
    dataType: 'json',
    success: function(data) {
      mostrar = data;
    },
    async: false
  });
  return mostrar;
}

/*-----------------Función Artistas Territorio -----------------------*/
function getArtistasTerritorio(id_usuario) {
  var mostrar = "";
  var datos = {
    'p1': Territorio
  };
  $.ajax({
    url: url_ok_obj+"getArtistasTerritorio",
    type: 'POST',
    data: datos,
    async: false
  }).done(function(data) {
    var table_artistas_territorio = null;
    if ($.fn.dataTable.isDataTable('#table_artistas_territorio')) {
      table_artistas_territorio = $('#table_artistas_territorio').DataTable().destroy();
    }
    $("#table_artistas_territorio tbody").html(data);
    if ($.fn.dataTable.isDataTable('#table_artistas_territorio')) {
      table_artistas_territorio = $('#table_artistas_territorio').DataTable();
    } else {
      table_artistas_territorio = $("#table_artistas_territorio").DataTable({
         autoWidth: false,
         responsive: false,
         pageLength: 100,
         paging: false,
         info: false,
         order: [[ 1, "asc" ]],
         dom: 'Bfrtip',
        "language": {
          "lengthMenu": "Ver _MENU_ registros por pagina",
          "zeroRecords": "No hay información, lo sentimos.",
          "info": "Mostrando pagina _PAGE_ de _PAGES_",
          "infoEmpty": "No hay registros disponibles",
          "search": "Filtrar"
        }
      });
    }
    table_artistas_territorio.draw();
  }).fail(function(data) {
    parent.mostrarAlerta("error","Mensaje del sistema", "No se han podido cargar los datos de la tabla");
  });
  return mostrar;
}


///////////////////////////////////////

var table_equipo_central = $("#table_equipo_central").DataTable({
  autoWidth: false,
  responsive: true,
  pageLength: 100,
  paging: false,
  dom: 'Bfrtip',
  buttons: [
    'excel'
  ],
  "language": {
    "lengthMenu": "Ver _MENU_ registros por pagina",
    "zeroRecords": "No hay información, lo sentimos.",
    "info": "Mostrando pagina _PAGE_ de _PAGES_",
    "infoEmpty": "No hay registros disponibles",
    "infoFiltered": "(filtered from _MAX_ total records)",
    "search": "Filtrar"
  }
});

/////////////////////////////////////
function getEquipoCentral() {
  parent.mostrarCargando();
  //table_equipo_central.clear().draw();
  var datos = {
    'p1': '20'
  };
  $.ajax({
    url: url_ok_obj+"getEquipoCentral",
    type: 'POST',
    data: datos,
    success: function(data) {
      console.log(data);
      table_equipo_central.rows.add($(data)).draw();
      parent.cerrarCargando();
    },
    async: true
  })
}










/*-----------------Función Artistas Territorio -----------------------*/
function getArtistasLaboratorio(id_usuario) {
  var mostrar = "";
  var datos = {
    'p1': Laboratorio
  };
  $.ajax({
    url: url_ok_obj+"getArtistasLaboratorio",
    type: 'POST',
    data: datos,
    async: false
  }).done(function(data) {
    var table_artistas_laboratorio = null;
    if ($.fn.dataTable.isDataTable('#table_artistas_laboratorio')) {
      table_artistas_laboratorio = $('#table_artistas_laboratorio').DataTable().destroy();
    }
    $("#table_artistas_laboratorio tbody").html(data);
    if ($.fn.dataTable.isDataTable('#table_artistas_laboratorio')) {
      table_artistas_laboratorio = $('#table_artistas_laboratorio').DataTable();
    } else {
      table_artistas_laboratorio = $("#table_artistas_laboratorio").DataTable({
         autoWidth: false,
         responsive: false,
         pageLength: 100,
         paging: false,
         info: false,
         order: [[ 1, "asc" ]],
         dom: 'Bfrtip',
        "language": {
          "lengthMenu": "Ver _MENU_ registros por pagina",
          "zeroRecords": "No hay información, lo sentimos.",
          "info": "Mostrando pagina _PAGE_ de _PAGES_",
          "infoEmpty": "No hay registros disponibles",
          "search": "Filtrar"
        }
      });
    }
    table_artistas_laboratorio.draw();
  }).fail(function(data) {
    parent.mostrarAlerta("error","Mensaje del sistema", "No se han podido cargar los datos de la tabla");
  });
  return mostrar;

}

  $("#form_nuevo_fortalecimiento").validate({
    rules: {
      SL_Localidad: {
        required: true,
      },
      SL_Lugar: {
        required: true,
      },
       TX_Evento: {
        required: true,
      },
      TX_Fecha_Encuentro1: {
        required: true
      },
    },
    messages: {
      SL_Localidad: {
        required: "Por favor seleccione la localidad",
      },
      SL_Lugar: {
        required: "Por favor seleccione el lugar",
       },
      TX_Evento: {
        required: "Por favor ingrese el nombre del fortalecimiento",
      },
      TX_Fecha_Encuentro1: {
        required: "Por favor seleccione un evento para que cargue la fecha",
      }
    },
    invalidHandler: function(event, validator) {
      return false;
    },
    errorPlacement: function(error, element) {
      $(element).closest('.form-group').find('.help-block').html(error.html());
    },
    highlight: function(element) {
      $(element).closest('.form-group').removeClass('has-success').addClass('has-error');
    },
    unhighlight: function(element, errorClass, validClass) {
      $(element).closest('.form-group').removeClass('has-error').addClass('has-success');
      $(element).closest('.form-group').find('.help-block').html('');
    },
    submitHandler: function(form) {
      parent.swal({
        confirmButtonColor: '#3f9a9d',
        title: 'Guardar Fortalecimiento',
        html: '<small>¿Realmente desea guardar el fortalecimiento?</small>',
        type: 'warning',
        confirmButtonText: 'Guardar FAP',
        cancelButtonText: 'Cancelar',
        showCancelButton: true,
        width: 800,
      }).then(() => {
        crearNuevoFortalecimiento();
      },(confirm) => {
        if (confirm == "cancel"){
        }
      }).catch(parent.swal.noop);
    }
  });

//********  para guardar una nueva Dupla
  function crearNuevoFortalecimiento() {

     var checkbox_asistencia_beneficiario = new Array();
    $('.asistencia_experiencia').each(function() {
      var check;
      if($(this).is(":checked")){
        check=1;
      }else{
        check=0;
      }
      checkbox_asistencia_beneficiario.push(new Array($(this).data('id_beneficiario'), check));
    });
   checkbox_asistencia_beneficiario = JSON.stringify(checkbox_asistencia_beneficiario);

    datos = {
      p1: {
        'Lugar': $("#SL_Lugar").val(),
        'Nom_Evento': $("#TX_Evento").val(),
        'Id_Usuario': parent.idUsuario,
        'Territorio': Territorio,
        'CH_asistencia_artista': checkbox_asistencia_beneficiario,
      }
    };
    $.ajax({
      url: url_ok_obj+"crearNuevoFortalecimientoController",
      type: 'POST',
      data: datos,
      success: function(data) {
        parent.mostrarAlerta("success","Operación exitosa","Fortalecimiento Creado Correctamente");
        limpiarFormulario();
      },
      async: false
    });

  }


  $("#form_nuevo_laboratorio").validate({
    rules: {
      SL_Localidad_LAB: {
        required: true,
      },
      SL_Lugar_LAB: {
        required: true,
      },
       TX_Evento_LAB: {
        required: true,
      },
      TX_Fecha_Encuentro1_LAB: {
        required: true
      },
    },
    messages: {
      SL_Localidad_LAB: {
        required: "Por favor seleccione la localidad",
      },
      SL_Lugar_LAB: {
        required: "Por favor seleccione el lugar",
       },
      TX_Evento_LAB: {
        required: "Por favor ingrese el nombre del fortalecimiento",
      },
      TX_Fecha_Encuentro1_LAB: {
        required: "Por favor seleccione un evento para que cargue la fecha",
      }
    },
    invalidHandler: function(event, validator) {
      return false;
    },
    errorPlacement: function(error, element) {
      $(element).closest('.form-group').find('.help-block').html(error.html());
    },
    highlight: function(element) {
      $(element).closest('.form-group').removeClass('has-success').addClass('has-error');
    },
    unhighlight: function(element, errorClass, validClass) {
      $(element).closest('.form-group').removeClass('has-error').addClass('has-success');
      $(element).closest('.form-group').find('.help-block').html('');
    },
    submitHandler: function(form) {
      parent.swal({
        confirmButtonColor: '#3f9a9d',
        title: 'Guardar laboratorio',
        html: '<small>¿Realmente desea guardar el laboratorio?</small>',
        type: 'warning',
        confirmButtonText: 'Guardar FAP',
        cancelButtonText: 'Cancelar',
        showCancelButton: true,
        width: 800,
      }).then(() => {
        crearNuevoLaboratorio();
      },(confirm) => {
        if (confirm == "cancel"){
        }
      }).catch(parent.swal.noop);
    }
  });

//******** Funcion para guardar una nueva Dupla
  function crearNuevoLaboratorio() {

     var checkbox_asistencia_artista = new Array();
    $('.asistencia_laboratorio').each(function() {
      var check;
      if($(this).is(":checked")){
        check=1;
      }else{
        check=0;
      }
      checkbox_asistencia_artista.push(new Array($(this).data('id_artista'), check));
    });
   checkbox_asistencia_artista = JSON.stringify(checkbox_asistencia_artista);

    datos = {
      p1: {
        'Lugar': $("#SL_Lugar_LAB").val(),
        'Nom_Evento': $("#TX_Evento_LAB").val(),
        'Id_Usuario': parent.idUsuario,
        'Territorio': Territorio,
        'CH_asistencia_artista': checkbox_asistencia_artista,
      }
    };
    $.ajax({
      url: url_ok_obj+"crearNuevoFortalecimientoController",
      type: 'POST',
      data: datos,
      success: function(data) {
        parent.mostrarAlerta("success","Operación exitosa","LaboratorioCreado Correctamente");
        limpiarFormulario_LAB();
      },
      async: false
    });

  }




  function ConsultarFortalecimientosEaat() {
    var mostrar = "";
    var datos = {
      'p1': parent.idUsuario,
      'p2': $("#SL_Mes_Consolidado").val()
    };
    $.ajax({
      url: url_ok_obj+"getFortalecimientosTerritorio",
      type: 'POST',
      data: datos,
      async: false
    }).done(function(data) {
      var table_fortalecimientos_territorio = null;
      if ($.fn.dataTable.isDataTable('#table_fortalecimientos_territorio')) {
        table_fortalecimientos_territorio = $('#table_fortalecimientos_territorio').DataTable().destroy();
      }
      $("#table_fortalecimientos_territorio tbody").html(data);
      if ($.fn.dataTable.isDataTable('#table_fortalecimientos_territorio')) {
        table_fortalecimientos_territorio = $('#table_fortalecimientos_territorio').DataTable();
      } else {
        table_fortalecimientos_territorio = $("#table_fortalecimientos_territorio").DataTable({
           autoWidth: false,
           responsive: false,
           pageLength: 100,
           paging: false,
           info: false,
           dom: 'Bfrtip',
          "language": {
            "lengthMenu": "Ver _MENU_ registros por pagina",
            "zeroRecords": "No hay información, lo sentimos.",
            "info": "Mostrando pagina _PAGE_ de _PAGES_",
            "infoEmpty": "No hay registros disponibles",
            "infoFiltered": "(filtered from _MAX_ total records)",
            "search": "Filtrar"
          }
        });
      }
      table_fortalecimientos_territorio.draw();
    }).fail(function(data) {
      parent.mostrarAlerta("error","Mensaje del sistema", "No se han podido cargar los datos de la tabla");
    });
    return mostrar;

  }

  function limpiarFormulario() {
    $(".has-error").removeClass("has-error");
    $(".has-success").removeClass("has-success");
    $('.help-block').html('');
    $("#SL_Localidad").val("");
    $("#SL_Localidad").selectpicker("refresh");
    $("#SL_Lugar").val("");
    $("#SL_Lugar").selectpicker("refresh");
    $("#TX_Evento").val("");
    $("#TX_Evento").selectpicker("refresh");
    $("#TX_Fecha_Encuentro1").val("");
  }

  function limpiarFormulario_LAB() {
    $(".has-error").removeClass("has-error");
    $(".has-success").removeClass("has-success");
    $('.help-block').html('');
    $("#SL_Localidad_LAB").val("");
    $("#SL_Localidad_LAB").selectpicker("refresh");
    $("#SL_Lugar_LAB").val("");
    $("#SL_Lugar_LAB").selectpicker("refresh");
    $("#TX_Evento_LAB").val("");
    $("#TX_Evento_LAB").selectpicker("refresh");
    $("#TX_Fecha_Encuentro1_LAB").val("");
  }

/*** PDF DEL FORTALECIMEINTO INDIVIDUAL  ***/

  function procesarInfoPDFInforme(idFortalecimiento) {
    var datosPDF = cargarEncabezadoReporte(idFortalecimiento);
    var beneficiaros = cargarArtistasFortalecimiento(idFortalecimiento);

      datosPDF.beneficiarosTabla = [];

      datosPDF.beneficiarosTabla.push(
      [{text:'IDENTIFICACIÓN',style: 'titles'},
      {text:'NOMBRE ARTISTA',style: 'titles'},
      {text:'LOCALIDAD',style: 'titles'},
      {text:'DUPLA',style: 'titles'},
      {text:'ASISTENCIA',style: 'titles'}],
    );
  beneficiaros.forEach(function(beneficiario) {

       datosPDF.beneficiarosTabla.push(
          [{text:beneficiario.IDENTIFICACION,style: 'campos'},{text:beneficiario.NOMBRE,style: 'campos'},{text:' - ',style: 'campos'},{text:beneficiario.DUPLA,style: 'campos'},{text:beneficiario.ASISTENCIA,style: 'campos'}],
        );
    });
    descargarPDFInforme(datosPDF);
  }

  function descargarPDFInforme(datosPDF) {
    var count = 0 ;
    var bandera = false;
    var dd = {

        pageMargins:[20,15,20,20],
        pageSize: 'letter',
        pageOrientation: 'landscape',
        header:
        {

        },

      content: [

        {
            margin: [0, 10, 0, 0],
          style: 'tableExample',
          table: {
              widths: [100,"*",100],
            body: [
            [{image: 'idartes_logo',width: 50,rowSpan: 2,style: 'titles',margin: [0, 2, 0, 0]},{text:'GESTIÓN PARA LA APROPIACIÓN DE LAS ARTÍSTICAS ATENCIÓN A PRIMERA INFANCIA',style: 'titles',margin: [0, 5, 0, 5]},{image: 'nidos_logo',width: 80,rowSpan: 2,style: 'titles',margin: [0, 2, 0, 0]}],
            [{},{text:'REPORTE FORTALECIMIENTO ARTISTICO PEDAGÓGICO',style: 'titles',margin: [0, 8, 0, 0]},{}],

            ]
          },layout: {
            vLineWidth: function (i, node) {
              return 0.5;
            },
            hLineWidth: function (i, node) {
              return 0.5;
            },
          }
        },
        {
            margin: [0, 0, 0, 0],
          style: 'tableExample',
          table: {
              widths: [178,178,178,"*"],
            body: [

            [{text:'EQUIPO TERRITORIAL',style: 'titles'},
            {text:'EAAT RESPONSABLE',style: 'titles'},
            {text:'NOMBRE EVENTO',style: 'titles'},
            {text:'FECHA EVENTO',style: 'titles'}],
            [{text:datosPDF.TERRITORIO,style: 'campos'},{text:datosPDF.EAAT,style: 'campos'},{text:datosPDF.NOMBRE,style: 'campos'},{text:datosPDF.FECHA,style: 'campos'}],

            [{text:'LUGAR EVENTO',style: 'titles', border: [true, false, true, true]},
            {text:'BARRIO',style: 'titles', border: [true, false, true, true]},
            {text:'UPZ',style: 'titles', border: [true, false, true, true]},
            {text:'LOCALIDAD',style: 'titles', border: [true, false, true, true]}],
            [{text:datosPDF.LUGAR,style: 'campos'},{text:datosPDF.BARRIO,style: 'campos'},{text:datosPDF.UPZ,style: 'campos'},{text:datosPDF.LOCALIDAD,style: 'campos'}],

           ]
          },layout: {
            vLineWidth: function (i, node) {
              return 0.5;
            },
            hLineWidth: function (i, node) {
              return 0.5;
            },
          }
        },
        {
            margin: [0, 0, 0, 0],
          style: 'tableExample',
          table: {
              widths: ["*"],
            body: [
            [{text:'LISTADO DE ASISTENCIA ARTISTAS COMUNITARIOS',style: 'titles', fillColor: '#dddddd', border: [true, true, true, true]}],
                  ]
          },layout: {
            vLineWidth: function (i, node) {
              return 1;
            },
            hLineWidth: function (i, node) {
              return 1;
            },
          }
        },
        {
            margin: [0, 0, 0, 0],
          style: 'tableExample',
          table: {
              widths: [143,186,186,100,"*"],
            body: datosPDF.beneficiarosTabla
          },layout: {
            vLineWidth: function (i, node) {
              return 0.5;
            },
            hLineWidth: function (i, node) {
              return 0.5;
            },
          }
        },
    ],


      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        titles:{
            bold: true,
            alignment:'center',
            fontSize: 7,
          color: 'black'
        },
        campos:{
            alignment:'center',
            fontSize: 7,
          color: 'black'
        },
        titles:{
            bold: true,
            alignment:'center',
            fontSize: 7,
          color: 'black'
        },
        tableHeader: {
          bold: true,
          fontSize: 13,
          color: 'black'
        }
      },


     images: {
        idartes_logo: 'data:image/png;base64,',

     nidos_logo: 'data:image/png;base64,'
      },

      defaultStyle: {
        // alignment: 'justify'
      }

}

  pdfMake.createPdf(dd).open();
  //pdfMake.createPdf(dd).download("ReporteAtencionGrupo.pdf");
  }

});
