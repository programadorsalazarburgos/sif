var url_ok_obj = '../../../src/PedagogicoNidos/ReporteFAPController/';  
var Territorio;

$(function() {

  //$("#SL_Mes_Consolidado").html(parent.getOptionsParametroDetalle(8)).selectpicker("refresh");
  $(".SeleccionarMes").html(parent.getOptionsParametroDetalle(8)).selectpicker("refresh");

  $("#SL_Territorio").html(getListadoTerritorio()).selectpicker("refresh");
  $("#SL_Localidad").html(getListadoLocalidades()).selectpicker("refresh");

  $("#SL_Mes_Consolidado").change(cargarDatosConsolidado);


  function cargarDatosConsolidado() {
    $("#div_consolidado_fap_mes").show("refresh");
    ConsultarConsolidadoFAP();
  }
  ...
  $("#BT_Consultar_FAP_Localidad").click(function(){
    $("#div_fap_mes_localidad").show("refresh");
    ConsultarFAPLocalidad();
  })
  
  $("#BT_Consultar_FAP").click(function(){
    $("#div_detallado_fap_mes").show("refresh");
    ConsultarDetalladoFAP();
  })

  function getListadoTerritorio() {
    var mostrar = "";
    $.ajax({
      url: url_ok_obj+"getTerritorios",
      type: 'POST',
  ...
  function getListadoLocalidades() {
    var mostrar = "";
    $.ajax({
      url: url_ok_obj+"getLocalidades",
      type: 'POST',
      success: function(data) {
        mostrar += data;
      },
  ...
  function ConsultarConsolidadoFAP() {
    var mostrar = "";
    var datos = {
      'p1': $("#SL_Mes_Consolidado").val()
    };
    $.ajax({
      url: url_ok_obj+"getConsolidadoFAP",
      type: 'POST',
      data: datos,
      async: false
    }).done(function(data) {
      var table_consolidado_FAP = null;
      if ($.fn.dataTable.isDataTable('#table_consolidado_FAP')) {
        table_consolidado_FAP = $('#table_consolidado_FAP').DataTable().destroy();
      }
      $("#table_consolidado_FAP tbody").html(data);
      if ($.fn.dataTable.isDataTable('#table_consolidado_FAP')) {
        table_consolidado_FAP = $('#table_consolidado_FAP').DataTable();
      } else {
        table_consolidado_FAP = $("#table_consolidado_FAP").DataTable({
          autoWidth: false,
          pageLength: 50,
  ...
  function ConsultarFAPLocalidad() {
    var mostrar = "";
    var datos = {
      'p1': $("#SL_Mes_Localidad").val(),
      'p2': $("#SL_Localidad").val()
    };
    $.ajax({
      url: url_ok_obj+"getDetalladoFAPLocalidad",
      type: 'POST',
      data: datos,
      async: false
    }).done(function(data) {
      var table_FAP_localidad = null;
      if ($.fn.dataTable.isDataTable('#table_FAP_localidad')) {
        table_FAP_localidad = $('#table_FAP_localidad').DataTable().destroy();
      }
      $("#table_FAP_localidad tbody").html(data);
      if ($.fn.dataTable.isDataTable('#table_FAP_localidad')) {
        table_FAP_localidad = $('#table_FAP_localidad').DataTable();
      } else {
        table_FAP_localidad = $("#table_FAP_localidad").DataTable({
          autoWidth: false,
          pageLength: 50,
  ...
  function ConsultarDetalladoFAP() {
    var mostrar = "";
    var datos = {
      'p1': $("#SL_Mes_Detallado").val(),
      'p2': $("#SL_Territorio").val()
    };
    $.ajax({
      url: url_ok_obj+"getDetalladoFAP",
      type: 'POST',
      data: datos,
      async: false
    }).done(function(data) {
      var table_detallado_FAP = null;
      if ($.fn.dataTable.isDataTable('#table_detallado_FAP')) {
        table_detallado_FAP = $('#table_detallado_FAP').DataTable().destroy();
      }
      $("#table_detallado_FAP tbody").html(data);
      if ($.fn.dataTable.isDataTable('#table_detallado_FAP')) {
        table_detallado_FAP = $('#table_detallado_FAP').DataTable();
      } else {
        table_detallado_FAP = $("#table_detallado_FAP").DataTable({
          autoWidth: false,
          pageLength: 50,
  ...
  function procesarInfoPDFInforme(idFortalecimiento) {
    ...
    beneficiaros.forEach(function(beneficiario) {
      ...
      datosPDF.beneficiarosTabla.push(
        [{text:beneficiario.IDENTIFICACION,style: 'campos'},{text:beneficiario.NOMBRE,style: 'campos'},{text:' - ',style: 'campos'},{text:' - ',style: 'campos'},{text:beneficiario.ASISTENCIA,style: 'campos'}],
        );
    });
