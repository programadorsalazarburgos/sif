var url_ok_obj = '../../../src/Reportes/Controlador/ConsolidadoMensualGestorController.php';  
 
$(function(){
$(".Dupla_Consultar").html(getConsultarDupla()).selectpicker("refresh");
$("#SL_Mes").html(getMes()).selectpicker("refresh");

$("#SL_Dupla_Cifras").html(getConsultarDupla()).selectpicker("refresh");
$("#SL_Mes_Cifras").html(getMes()).selectpicker("refresh");

  $("table").delegate(".experiencias_sin", "click", function() {

    $("#id_expe").val($(this).data("id-evento"));
    $("#grupo").html($(this).data("evento"));
    $("#experiencias").html($(this).data("experiencia"));

    $("#real3").html($(this).data("real3"));
    $("#real5").html($(this).data("real5"));
    $("#real6").html($(this).data("real6"));    
    $("#realgestantes").html($(this).data("realgestantes"));
    $("#realdiscapidad").html($(this).data("realdiscapidad"));    
    $("#total").html($(this).data("total"));
    $("#nuevos3").html($(this).data("nuevos3"));
    $("#nuevos5").html($(this).data("nuevos5"));
    $("#nuevos6").html($(this).data("nuevos6"));    
    $("#nuevosgestantes").html($(this).data("nuevosgestantes"));
    $("#nuevosdiscapidad").html($(this).data("nuevosdiscapidad"));    
    $("#nuevos").html($(this).data("nuevos"));   

    var idEventoAprobar = $(this).data("id-evento");
    console.log(idEventoAprobar);
  });



    $("#Generar_Reporte").click(function() {
       var idMesReporte = $("#SL_Mes_Cifras").val();
       var idDupla = $("#SL_Dupla_Cifras").val();
       procesarInfoPDFMensual(idMesReporte, idDupla);
    });

  $("table").delegate(".consultar_beneficiarios", "click", function() {
    var beneficiarios = $(this).data("id_beneficiairos");
    consultarBeneficiariosNuevos(beneficiarios);
    console.log(beneficiarios);  
  });    


//******** Consulta Duplas para Modificar
function getConsultarDupla() {
  var mostrar = "";
  var datos = {
    funcion: 'getConsultarDuplaM',
    'p1': parent.idUsuario
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    success: function(data) {
      mostrar += data;
    },
    async: false
  });
  return mostrar;
}

function getMes() {
  var mostrar = "";
  var datos = {
    funcion: 'getOptionsMes'
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    success: function(data) {
      mostrar += data
    },
    async: false
  });
  return mostrar;
}


/*-----------------CARGAR ENCABEZADO DEL REPORTE MENSUAL-----------------------*/
function cargarEncabezadoReporteMensual() {
  var mostrar;
  var datos = {
    funcion: 'consultarEncabezadoReporteMensual',
    'p1': $("#SL_Dupla_Cifras").val()
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    dataType: 'json',
    success: function(data) {
      mostrar = data[0];
    },
    async: false
  });
  return mostrar;
}

/*-----------------CARGAR CUERPO DEL REPORTE MENSUAL-----------------------*/
function cargarTotalesReporteMensual() {
  var mostrar;
  var datos = {
    funcion: 'consultarTotalesMensualPdf',
    'p1': $("#SL_Dupla_Cifras").val(),
    'p2': $("#SL_Mes_Cifras").val()
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    dataType: 'json',
    success: function(data) {
      mostrar = data[0];
    },
    async: false
  });
  return mostrar;
}


/*-----------------CARGAR CUERPO DEL REPORTE MENSUAL-----------------------*/
function cargarCuerpoReporteMensual() {
  var $mes_consulta = $("#SL_Mes_Cifras").val();
  var $mes_anterior = $mes_consulta - 1;
  var mostrar;
  var datos = {
    funcion: 'consultarTotalAtencionesMensualPdf',
    'p1': $("#SL_Dupla_Cifras").val(),
    'p2': $("#SL_Mes_Cifras").val(),
    'p3': $mes_anterior
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    dataType: 'json',
    success: function(data) {
      mostrar = data[0];
    },
    async: false
  });
  return mostrar;
}

function cargarCuerpoReporteMensualCifras() {
  var $mes_consulta = $("#SL_Mes_Cifras").val();
  var $mes_anterior = $mes_consulta - 1;
  var mostrar;
  var datos = {
    funcion: 'consultarTotalAtencionesMensualCifrasPdf',
    'p1': $("#SL_Dupla_Cifras").val(),
    'p2': $("#SL_Mes_Cifras").val(),
    'p3': $mes_anterior
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    dataType: 'json',
    success: function(data) {
      mostrar = data[0];
    },
    async: false
  });
  return mostrar;
}

  $("#BT_Consultar_Pandora").click(function(){
    $("#div_Consolidado").show();
    ConsultaConsolidadoMensual();
  });

  var table_lugar_atencion = $("#table-lugar-atencion").DataTable({
    autoWidth: false,
    responsive: true,
    pageLength: 100,
    paging: false,
    dom: 'Bfrtip',
    buttons: [
      'excel'
    ],
    "language": {
      "lengthMenu": "Ver _MENU_ registros por pagina",
      "zeroRecords": "No hay información, lo sentimos.",
      "info": "Mostrando pagina _PAGE_ de _PAGES_",
      "infoEmpty": "No hay registros disponibles",
      "infoFiltered": "(filtered from _MAX_ total records)",
      "search": "Filtrar"
    }
  }); 

  var table_lugar_laboratorio = $("#table-lugar-laboratorio").DataTable({
    autoWidth: false,
    responsive: true,
    pageLength: 100,
    paging: false,
    dom: 'Bfrtip',
    buttons: [
      'excel'
    ],
    "language": {
      "lengthMenu": "Ver _MENU_ registros por pagina",
      "zeroRecords": "No hay información, lo sentimos.",
      "info": "Mostrando pagina _PAGE_ de _PAGES_",
      "infoEmpty": "No hay registros disponibles",
      "infoFiltered": "(filtered from _MAX_ total records)",
      "search": "Filtrar"
    }
  }); 


  var table_beneficiarios_enfoque = $("#table-beneficiarios-enfoque").DataTable({
  autoWidth: false,
  responsive: true,
  pageLength: 100,
  paging: false,
  dom: 'Bfrtip',
  buttons: [
    'excel'
  ],
  "language": {
    "lengthMenu": "Ver _MENU_ registros por pagina",
    "zeroRecords": "No hay información, lo sentimos.",
    "info": "Mostrando pagina _PAGE_ de _PAGES_",
    "infoEmpty": "No hay registros disponibles",
    "infoFiltered": "(filtered from _MAX_ total records)",
    "search": "Filtrar"
  }
}); 

tabla_config = {
  paging: false,
  scrollX: true,
  scrollCollapse: true,
  fixedColumns:{
    leftColumns: 6,
  },
  scrollY: "600px",
  dom: 'Bfrtip',
  buttons: [
  'excel'
  ],
  "language": {
    "lengthMenu": "Ver _MENU_ registros por página",
    "zeroRecords": "No hay información, lo sentimos.",
    "info": "Mostrando página _PAGE_ de _PAGES_",
    "infoEmpty": "No hay registros disponibles",
    "infoFiltered": "(filtered from _MAX_ total records)",
    "search": "Filtrar"
  }
};

table_reporte_evento = $("#table-reporte-evento").DataTable(tabla_config);

//var table_beneficiarios_enfoque = $("#table-beneficiarios-enfoque").DataTable(tabla_config);

$("#BT_Experiencias_Cifras").click(function(){
  $("#div-reporte-evento").show();
  consultarTotalExperienciasMes();
  consultarExperienciaLugarDupla();
  consultarExperienciaLaboratorioDupla();
  getExperienciasCifras();
});


$("table").delegate(".EnfoqueReal", "click", function() {
  //$("#id_expe").val($(this).data("id-experiencia"));
  var idExpeEnfoque = $(this).data("id-experiencia");
  console.log(idExpeEnfoque);
  consultarBeneficiariosEnfoque(idExpeEnfoque);
  
});

/////////////////////////////////////

function consultarTotalExperienciasMes() {
  var mostrar = "";
  datos = {
    'funcion': 'consultarTotalExperienciasMes',
    'p1': $("#SL_Mes_Cifras").val(),
    'p2': $("#SL_Dupla_Cifras").val()
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    async: false,
    beforeSend: function() {}
  }).done(function(data) {
    try {
      datos = $.parseJSON(data);
      $("#SP_Total_Atenciones").text(datos['TOTAL']);
    } catch (ex) {
      parent.mostrarAlerta("error","Error","No se han podido cargar los datos del grupo");
    }
  }).fail(function(data) {
    parent.mostrarAlerta("error","Error","No se ha podido precargar los datos del lugar que desea modificar");
  });
}

/////////////////////////////////////////

function consultarExperienciaLugarDupla() {
  parent.mostrarCargando();
  table_lugar_atencion.clear().draw();
  var datos = {
    funcion: 'consultarExperienciaLugarDupla',
    'p1': $("#SL_Mes_Cifras").val(),
    'p2': $("#SL_Dupla_Cifras").val()
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    success: function(data) {
      table_lugar_atencion.rows.add($(data)).draw();
      parent.cerrarCargando();
    },
    async: true
  })
}

/////////////////////////////////////////

function consultarExperienciaLaboratorioDupla() {
  parent.mostrarCargando();
  table_lugar_laboratorio.clear().draw();
  var datos = {
    funcion: 'consultarExperienciaLaboratorioDupla',
    'p1': $("#SL_Mes_Cifras").val(),
    'p2': $("#SL_Dupla_Cifras").val()
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    success: function(data) {
      table_lugar_laboratorio.rows.add($(data)).draw();
      parent.cerrarCargando();
    },
    async: true
  })
}


/////////////////////////////////////

function getExperienciasCifras() {
  parent.mostrarCargando();
  table_reporte_evento.clear().draw();
  var datos = {
    funcion: 'getExperienciasDupla',
    'p1': $("#SL_Mes_Cifras").val(),
    'p2': $("#SL_Dupla_Cifras").val()
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    success: function(data) {
      table_reporte_evento.rows.add($(data)).draw();
      parent.cerrarCargando();
    },
    async: true
  })
}

///////////////////////////////////// MODAL ENFOQUE

function consultarBeneficiariosEnfoque(idExpeEnfoque) {
  parent.mostrarCargando();
  table_beneficiarios_enfoque.clear().draw();
  var datos = {
    funcion: 'getBeneficiariosEnfoque',
    'p1': idExpeEnfoque
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    success: function(data) {
      table_beneficiarios_enfoque.rows.add($(data)).draw();
      parent.cerrarCargando();
    },
    async: true
  })
} 




///////////////////////////////////////

var table_consolidado_mensual = $("#table_consolidado_mensual").DataTable({
  autoWidth: false,
  responsive: true,
  pageLength: 100,
  paging: false,
  dom: 'Bfrtip',
  buttons: [
    'excel'
  ],
  "language": {
    "lengthMenu": "Ver _MENU_ registros por pagina",
    "zeroRecords": "No hay información, lo sentimos.",
    "info": "Mostrando pagina _PAGE_ de _PAGES_",
    "infoEmpty": "No hay registros disponibles",
    "infoFiltered": "(filtered from _MAX_ total records)",
    "search": "Filtrar"
  }
});

/////////////////////////////////////
function ConsultaConsolidadoMensual() {
  parent.mostrarCargando();
  table_consolidado_mensual.clear().draw();
  var datos = {
    funcion: 'getConsolidadoMensualGestor',
    'p1': $("#SL_Dupla_Consultar").val(),
    'p2': $("#SL_Mes").val()
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    success: function(data) {
      table_consolidado_mensual.rows.add($(data)).draw();
      parent.cerrarCargando();
    },
    async: true
  })
}

/////////////////////////////////////// BENEFICIARIOS NUEVOS EN EL GRUPO
var tabla_beneficiarios_nuevos = $("#tabla_beneficiarios_nuevos").DataTable({
  autoWidth: false,
  responsive: true,
  pageLength: 100,
  paging: false,
  dom: 'Bfrtip',
  buttons: [
    'excel'
  ],
  "language": {
    "lengthMenu": "Ver _MENU_ registros por pagina",
    "zeroRecords": "No hay información, lo sentimos.",
    "info": "Mostrando pagina _PAGE_ de _PAGES_",
    "infoEmpty": "No hay registros disponibles",
    "infoFiltered": "(filtered from _MAX_ total records)",
    "search": "Filtrar"
  }
});
function consultarBeneficiariosNuevos(beneficiarios) {
  parent.mostrarCargando();
  tabla_beneficiarios_nuevos.clear().draw();
  var datos = {
    funcion: 'consultarBeneficiariosNuevos',
    'p1': beneficiarios
  };
  $.ajax({
    url: url_ok_obj,
    type: 'POST',
    data: datos,
    success: function(data) {
      $("#info_beneficiarios").html("").html(data);
      /*tabla_beneficiarios_nuevos.rows.add($(data)).draw();*/
      parent.cerrarCargando();
    },
    async: true
  })
}






 ///****************************  APROBAR ATENCIONES
  $("#form_aprobar_experiencia").submit(function(event) {
    event.stopPropagation();
    event.preventDefault();
    var mostrar = "";
    datos = {
      funcion: 'AprobarExperiencia',
      p1: {
        'Id_Experiencia': $("#id_expe").val()
      }
    };
    $.ajax({
      url: url_ok_obj,
      type: 'POST',
      data: datos,
      success: function(data) {
        mostrar += data;
        parent.mostrarAlerta("success","Operación exitosa","Se aprobo correctamente la del grupo para este mes");
        $("#miModalAprobacion").modal('hide');
        $("#div-reporte-evento").show("refresh");
        getExperienciasCifras();
      },
      async: false
    });
    return mostrar;
  });

  function procesarInfoPDFMensual(idMesReporte, idDupla) {
    var datosPDFMensual = cargarEncabezadoReporteMensual(idMesReporte);
    var AsistenciaPDFMensual = cargarCuerpoReporteMensual(idMesReporte, idDupla);
    var AsistenciaPDFMensualCifras = cargarCuerpoReporteMensualCifras(idMesReporte, idDupla);    
    var totalesMensual = cargarTotalesReporteMensual(idMesReporte, idDupla);

    descargarPDFMensual(datosPDFMensual, AsistenciaPDFMensual, AsistenciaPDFMensualCifras, totalesMensual);
  }


function descargarPDFMensual(datosPDFMensual, AsistenciaPDFMensual, AsistenciaPDFMensualCifras, totalesMensual) {
  var count = 0 ;
  var bandera = false;
  var dd = {
      pageMargins:[20,15,20,20],
      pageSize: 'letter',
      watermark: {text: 'APROBADO', color: 'gray', opacity: 0.2, bold: true, italics: false,marginX:0,marginY:60},
     // pageOrientation: 'landscape',
      header:
      {
      },

    content: [
      {
          margin: [0, 10, 0, 0],
          style: 'tableExample',
          table: {
          widths: [100,"*",100],
          body: [
          [{image: 'idartes_logo',width: 50,rowSpan: 2,style: 'campos',margin: [0, 2, 0, 0]},{text:'ARTE PARA LA PRIMERA INFANCIA - PROGRAMA NIDOS IDARTES',style: 'titles1',margin: [0, 5, 0, 5]},{image: 'nidos_logo',width: 80,rowSpan: 2,style: 'campos',margin: [0, 2, 0, 0]}],
          [{},{text:'CONSOLIDADO MENSUAL POR DUPLA',style: 'titles1',margin: [0, 8, 0, 0]},{}],
          ]
        },layout: {
          vLineWidth: function (i, node) {
          return 0.5;
        },
         hLineWidth: function (i, node) {
          return 0.5;
        },
        }
      },
    {
             margin: [0, 0, 0, 0],
            style: 'tableExample',
            table: {
                widths: [135,135,135,'*'],
              
              body: [
              [{text:'CÓDIGO DE DUPLA:',style: 'titles', border: [true, false, true, true]},
              {text:datosPDFMensual.CODIGODUPLA,style: 'campos', border: [true, false, true, true]},
              {text:'ARTISTAS:',style: 'titles', border: [true, false, true, true]},
              {text:datosPDFMensual.ARTISTAS,style: 'campos', border: [true, false, true, true]}],

              [{text:'TIPO DE DUPLA:',style: 'titles', border: [true, false, true, true]},
              {text:datosPDFMensual.TIPODUPLA,style: 'campos', border: [true, false, true, true]},
              {text:'TERRITORIO:',style: 'titles', border: [true, false, true, true]},
              {text:datosPDFMensual.TERRITORIO,style: 'campos', border: [true, false, true, true]}],

              [{text:'GESTOR:',style: 'titles', border: [true, false, true, true]},
              {text:datosPDFMensual.GESTOR,style: 'campos', border: [true, false, true, true]},
              {text:'EAAT:',style: 'titles', border: [true, false, true, true]},
              {text:datosPDFMensual.EAAT,style: 'campos', border: [true, false, true, true]}],
              ]

            },layout: {
              vLineWidth: function (i, node) {
                return 0.5;
              },
              hLineWidth: function (i, node) {
                return 0.5;
              },
            }
          },

    {text: '', style: 'subheader'},
          {

             margin: [0, 0, 0, 0],
            style: 'tableExample',
            table: {
                widths: [180,180,'*'],

              body: [
              [{text:'NÚMERO DE GRUPOS',style: 'titlesA', border: [true, true, true, true]},
              {text:'NÚMERO DE EXPERIENCIAS',style: 'titlesA', border: [true, true, true, true]},
             /* {text:'BENEFICIARIOS ATENDIDOS',style: 'titles', border: [true, true, true, true]}, */
              {text:'MES DE REPORTE',style: 'titlesA', border: [true, true, true, true]}],

              [{text:totalesMensual.GRUPOS,style: 'campos', border: [true, false, true, true]},
              {text:totalesMensual.EXPERIENCIAS,style: 'campos', border: [true, false, true, true]},
             /* {text:AsistenciaPDFMensual.TOTAL_R,style: 'campos', border: [true, false, true, true]}, */
              {text:totalesMensual.MES,style: 'campos', border: [true, false, true, true]}],
              ]

            },layout: {
              vLineWidth: function (i, node) {
                return 0.5;
              },
              hLineWidth: function (i, node) {
                return 0.5;
              },
            }
          },


          {text: '', style: 'subheader'},
          {
            margin: [0, 0, 0, 0],
           style: 'tableExample',
           table: {

             widths: [130,130,130,'*'],

             body: [
             [{text:'ATENCIONES NIÑO A NIÑO',style: 'titles2Nino', colSpan: 2,  border: [true, true, true, true]},
             {},
             {text:'ATENCIONES PROTECCIÓN DE DATOS',style: 'titles2Cifras', colSpan: 2, border: [true, true, true, true]},
             {}],             

             [{text:'Beneficiarios Nuevos',style: 'titles3Nino', border: [true, true, true, true]},
             {text:'Atención Real',style: 'titles3Nino', border: [true, true, true, true]},
             {text:'Beneficiarios Nuevos',style: 'titles3Cifras', border: [true, true, true, true]},
             {text:'Atención Real',style: 'titles3Cifras', border: [true, true, true, true]}],

             [{text:AsistenciaPDFMensual.TOTAL_N,style: 'titles2Verde',  border: [true, false, true, true]},              
             {text:AsistenciaPDFMensual.TOTAL_R,style: 'titles2Rojo',  border: [true, false, true, true]},              
             {text:AsistenciaPDFMensualCifras._TOTAL_N,style: 'titles2Verde', border: [true, false, true, true]},
             {text:AsistenciaPDFMensualCifras._TOTAL_R,style: 'titles2Rojo', border: [true, false, true, true]}],

             ]

           },layout: {
             vLineWidth: function (i, node) {
               return 0.5;
             },

             hLineWidth: function (i, node) {
               return 0.5;
             },
           }
         },





      {text: 'BENEFICIARIOS NUEVOS', style: 'subheader'},
      {text: '* Rango de Edad', style: 'subtitles'},

          {
             margin: [0, 0, 0, 0],
            style: 'tableExample',
            table: {

              widths: [60,60,60,60,60,60,60,'*'],

              body: [
              [{text:'TOTAL 0 - 3 AÑOS:',style: 'titles2Nuevos', colSpan: 2,  border: [true, true, true, true]},
              {},
              {text:'TOTAL 4 - 5 AÑOS',style: 'titles2Nuevos', colSpan: 2, border: [true, true, true, true]},
              {},
              {text:'TOTAL 6 AÑOS',style: 'titles2Nuevos', colSpan: 2, border: [true, true, true, true]},
              {},
              {text:'GESTANTES',style: 'titles2Nuevos', border: [true, true, true, true]},
              {text:'TOTAL BENEFICIARIOS:',style: 'titles2Nuevos', border: [true, true, true, true]}         
              ],

              [{text:AsistenciaPDFMensual.TOTAL_0_3_N,style: 'campos', colSpan: 2, border: [true, false, true, true]},
              {},
              {text:AsistenciaPDFMensual.TOTAL_4_5_N,style: 'campos', colSpan: 2, border: [true, false, true, true]},
              {},
              {text:AsistenciaPDFMensual.TOTAL_6_N,style: 'campos', colSpan: 2, border: [true, false, true, true]},
              {},
              {text:AsistenciaPDFMensual.GESTANTES_N,style: 'campos', rowSpan: 3, border: [true, false, true, true]},
              {text:AsistenciaPDFMensual.TOTAL_N,style: 'campos', rowSpan: 3, border: [true, false, true, true]}],

              [{text:'Niños de 0 a 3',style: 'titles2Nuevos', border: [true, true, true, true]},
              {text:'Niñas de 4 a 6',style: 'titles2Nuevos', border: [true, true, true, true]},
              {text:'Niños de 0 a 3',style: 'titles2Nuevos', border: [true, true, true, true]},
              {text:'Niñas de 4 a 6',style: 'titles2Nuevos', border: [true, true, true, true]},
              {text:'Niños de 0 a 3',style: 'titles2Nuevos', border: [true, true, true, true]},
              {text:'Niñas de 4 a 6',style: 'titles2Nuevos', border: [true, true, true, true]},
              {},{}],

              [{text:AsistenciaPDFMensual._0_3_NINOS_N,style: 'campos',  border: [true, false, true, true]},              
              {text:AsistenciaPDFMensual._0_3_NINAS_N,style: 'campos',  border: [true, false, true, true]},              
              {text:AsistenciaPDFMensual._4_5_NINOS_N,style: 'campos', border: [true, false, true, true]},
              {text:AsistenciaPDFMensual._4_5_NINAS_N,style: 'campos', border: [true, false, true, true]},
              {text:AsistenciaPDFMensual._6_NINOS_N,style: 'campos', border: [true, false, true, true]},
              {text:AsistenciaPDFMensual._6_NINAS_N,style: 'campos', border: [true, false, true, true]},
              {},{}],

              ]

            },layout: {
              vLineWidth: function (i, node) {
                return 0.5;
              },

              hLineWidth: function (i, node) {
                return 0.5;
              },
            }
          },

          {text: '* Enfoque Diferencial', style: 'subtitles'},

          {
            margin: [0, 0, 0, 0],
            style: 'tableExample',
            table: {

               widths: [80,80,80,80,80,'*'],

               body: [
                [{text:'Afrodescendientes',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Comunidad rural y campesina',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Comunidades negras',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Emergencia social y catastrófica',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'En zonas de deterioro urbano',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Población víctima del conflicto',style: 'titles2Nuevos', border: [true, true, true, true]}],
   
                [{text:AsistenciaPDFMensual.AFRO_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.CAMPESINA_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.NEGRAS_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.CATASTROFICA_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.DETERIORO_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.CONFLICTO_N,style: 'campos', border: [true, true, true, true]}],
   
                [{text:'Palenquera:',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Población carcelaria',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Situación de desplazamiento',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Habitantes de calle',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Actividades sexuales pagadas',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Consumidoras de sustancias psicoactivas',style: 'titles2Nuevos', border: [true, true, true, true]}],
   
                [{text:AsistenciaPDFMensual.PALENQUERA_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.CARCELARIA_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.DESPLAZAMIENTO_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.CALLE_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.SEXUALES_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.PSICOACTIVAS_N,style: 'campos', border: [true, true, true, true]}],
   
                [{text:'Población migrante',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Indígena',style: 'titles2Nuevos', border: [true, true, true, true]},                
                {text:'Raizal',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Rrom',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Sectores LGBTIQ',style: 'titles2Nuevos', border: [true, true, true, true]},
                {text:'Condición de discapacidad',style: 'titles2Nuevos', border: [true, true, true, true]}],
   
                [{text:AsistenciaPDFMensual.MIGRANTE_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.INDIGENA_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.RAIZAL_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.ROM_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.LGBTIQ_N,style: 'campos', border: [true, true, true, true]},
                {text:AsistenciaPDFMensual.DISCAPACIDAD_N,style: 'campos', border: [true, true, true, true]}],
               ]

           },layout: {
             vLineWidth: function (i, node) {
               return 0.5;
             },
             hLineWidth: function (i, node) {
               return 0.5;
             },
           }
         },    


      {text: 'BENEFICIARIOS ATENCIÓN REAL', style: 'subheader'},  
      {text: '* Rango de Edad', style: 'subtitles'},

      {
        margin: [0, 0, 0, 0],
       style: 'tableExample',
       table: {

           widths: [60,60,60,60,60,60,60,'*'],

         body: [
         [{text:'TOTAL 0 - 3 AÑOS:',style: 'titles2Real', colSpan: 2,  border: [true, true, true, true]},
         {},
         {text:'TOTAL 4 - 5 AÑOS',style: 'titles2Real', colSpan: 2, border: [true, true, true, true]},
         {},
         {text:'TOTAL 6 AÑOS',style: 'titles2Real', colSpan: 2, border: [true, true, true, true]},
         {},
         {text:'GESTANTES',style: 'titles2Real', border: [true, true, true, true]},
         {text:'TOTAL BENEFICIARIOS:',style: 'titles2Real', border: [true, true, true, true]}         
         ],

         [{text:AsistenciaPDFMensual.TOTAL_0_3_R,style: 'campos', colSpan: 2, border: [true, false, true, true]},
         {},
         {text:AsistenciaPDFMensual.TOTAL_4_5_R,style: 'campos', colSpan: 2, border: [true, false, true, true]},
         {},
         {text:AsistenciaPDFMensual.TOTAL_6_R,style: 'campos', colSpan: 2, border: [true, false, true, true]},
         {},
         {text:AsistenciaPDFMensual.GESTANTES_R,style: 'campos', rowSpan: 3, border: [true, false, true, true]},
         {text:AsistenciaPDFMensual.TOTAL_R,style: 'campos', rowSpan: 3, border: [true, false, true, true]}],

         [{text:'Niños de 0 a 3',style: 'titles2Real', border: [true, true, true, true]},
         {text:'Niñas de 0 a 3',style: 'titles2Real', border: [true, true, true, true]},
         {text:'Niños de 4 a 5',style: 'titles2Real', border: [true, true, true, true]},
         {text:'Niñas de 4 a 5',style: 'titles2Real', border: [true, true, true, true]},
         {text:'Niños 6',style: 'titles2Real', border: [true, true, true, true]},
         {text:'Niñas 6',style: 'titles2Real', border: [true, true, true, true]},
         {},{}],

         [{text:AsistenciaPDFMensual._0_3_NINOS_R,style: 'campos',  border: [true, false, true, true]},              
         {text:AsistenciaPDFMensual._0_3_NINAS_R,style: 'campos',  border: [true, false, true, true]},              
         {text:AsistenciaPDFMensual._4_5_NINOS_R,style: 'campos', border: [true, false, true, true]},
         {text:AsistenciaPDFMensual._4_5_NINAS_R,style: 'campos', border: [true, false, true, true]},
         {text:AsistenciaPDFMensual._6_NINOS_R,style: 'campos', border: [true, false, true, true]},
         {text:AsistenciaPDFMensual._6_NINAS_R,style: 'campos', border: [true, false, true, true]},
         {},{}],

         ]

       },layout: {
         vLineWidth: function (i, node) {
           return 0.5;
         },

         hLineWidth: function (i, node) {
           return 0.5;
         },
       }
     },


      {text: '* Enfoque Diferencial', style: 'subtitles'},

           {
             margin: [0, 0, 0, 0],
             style: 'tableExample',
             table: {

              widths: [80,80,80,80,80,'*'],

                body: [
                  [{text:'Afrodescendientes',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Comunidad rural y campesina',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Comunidades negras',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Emergencia social y catastrófica',style: 'titles2Real', border: [true, true, true, true]},
                {text:'En zonas de deterioro urbano',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Población víctima del conflicto',style: 'titles2Real', border: [true, true, true, true]}],
     
                  [{text:AsistenciaPDFMensual.AFRO_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.CAMPESINA_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.NEGRAS_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.CATASTROFICA_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.DETERIORO_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.CONFLICTO_R,style: 'campos', border: [true, true, true, true]}],
     
                [{text:'Palenquera:',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Población carcelaria',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Situación de desplazamiento',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Habitantes de calle',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Actividades sexuales pagadas',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Consumidoras de sustancias psicoactivas',style: 'titles2Real', border: [true, true, true, true]}],
     
                  [{text:AsistenciaPDFMensual.PALENQUERA_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.CARCELARIA_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.DESPLAZAMIENTO_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.CALLE_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.SEXUALES_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.PSICOACTIVAS_R,style: 'campos', border: [true, true, true, true]}],
     
                [{text:'Población migrante',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Indígena',style: 'titles2Real', border: [true, true, true, true]},                
                {text:'Raizal',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Rrom',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Sectores LGBTIQ',style: 'titles2Real', border: [true, true, true, true]},
                {text:'Condición de discapacidad',style: 'titles2Real', border: [true, true, true, true]}],
     
                  [{text:AsistenciaPDFMensual.MIGRANTE_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.INDIGENA_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.RAIZAL_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.ROM_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.LGBTIQ_R,style: 'campos', border: [true, true, true, true]},
                  {text:AsistenciaPDFMensual.DISCAPACIDAD_R,style: 'campos', border: [true, true, true, true]}],
                 ]

            },layout: {
              vLineWidth: function (i, node) {
                return 0.5;
              },
              hLineWidth: function (i, node) {
                return 0.5;
              },
            }
          },

          {text: '_______________________________________________________________________________', style: 'subheader'},
          
      {text: 'ATENCIONES PROTECCIÓN DE DATOS', style: 'subheader'},  
      {text: '* Beneficiarios Nuevos', style: 'subtitles'},

      {
        margin: [0, 0, 0, 0],
       style: 'tableExample',
       table: {

         widths: [60,60,60,60,60,60,60,'*'],

         body: [
         [{text:'TOTAL 0 - 3 AÑOS:',style: 'titles2Nuevos', colSpan: 2,  border: [true, true, true, true]},
         {},
         {text:'TOTAL 4 - 5 AÑOS',style: 'titles2Nuevos', colSpan: 2, border: [true, true, true, true]},
         {},
         {text:'TOTAL 6 AÑOS',style: 'titles2Nuevos', colSpan: 2, border: [true, true, true, true]},
         {},
         {text:'GESTANTES',style: 'titles2Nuevos', border: [true, true, true, true]},
         {text:'TOTAL BENEFICIARIOS:',style: 'titles2Nuevos', border: [true, true, true, true]}         
         ],

         [{text:AsistenciaPDFMensualCifras._0_3_TOTAL_N,style: 'campos', colSpan: 2, border: [true, false, true, true]},
         {},
         {text:AsistenciaPDFMensualCifras._4_5_TOTAL_N,style: 'campos', colSpan: 2, border: [true, false, true, true]},
         {},
         {text:AsistenciaPDFMensualCifras._6_TOTAL_N,style: 'campos', colSpan: 2, border: [true, false, true, true]},
         {},
         {text:AsistenciaPDFMensualCifras._GESTANTES_N,style: 'campos', rowSpan: 3, border: [true, false, true, true]},
         {text:AsistenciaPDFMensualCifras._TOTAL_N,style: 'campos', rowSpan: 3, border: [true, false, true, true]}],

         [{text:'Niños de 0 a 3',style: 'titles2Nuevos', border: [true, true, true, true]},
         {text:'Niñas de 4 a 6',style: 'titles2Nuevos', border: [true, true, true, true]},
         {text:'Niños de 0 a 3',style: 'titles2Nuevos', border: [true, true, true, true]},
         {text:'Niñas de 4 a 6',style: 'titles2Nuevos', border: [true, true, true, true]},
         {text:'Niños de 0 a 3',style: 'titles2Nuevos', border: [true, true, true, true]},
         {text:'Niñas de 4 a 6',style: 'titles2Nuevos', border: [true, true, true, true]},
         {},{}],

         [{text:AsistenciaPDFMensualCifras._0_3_NINOS_N,style: 'campos',  border: [true, false, true, true]},              
         {text:AsistenciaPDFMensualCifras._0_3_NINAS_N,style: 'campos',  border: [true, false, true, true]},              
         {text:AsistenciaPDFMensualCifras._4_5_NINOS_N,style: 'campos', border: [true, false, true, true]},
         {text:AsistenciaPDFMensualCifras._4_5_NINAS_N,style: 'campos', border: [true, false, true, true]},
         {text:AsistenciaPDFMensualCifras._6_NINOS_N,style: 'campos', border: [true, false, true, true]},
         {text:AsistenciaPDFMensualCifras._6_NINAS_N,style: 'campos', border: [true, false, true, true]},
         {},{}],

         ]

       },layout: {
         vLineWidth: function (i, node) {
           return 0.5;
         },

         hLineWidth: function (i, node) {
           return 0.5;
         },
       }
     },

     {text: '* Atención Real', style: 'subtitles'},

     {
       margin: [0, 0, 0, 0],
      style: 'tableExample',
      table: {

          widths: [60,60,60,60,60,60,60,'*'],

        body: [
        [{text:'TOTAL 0 - 3 AÑOS:',style: 'titles2Real', colSpan: 2,  border: [true, true, true, true]},
        {},
        {text:'TOTAL 4 - 5 AÑOS',style: 'titles2Real', colSpan: 2, border: [true, true, true, true]},
        {},
        {text:'TOTAL 6 AÑOS',style: 'titles2Real', colSpan: 2, border: [true, true, true, true]},
        {},
        {text:'GESTANTES',style: 'titles2Real', border: [true, true, true, true]},
        {text:'TOTAL BENEFICIARIOS:',style: 'titles2Real', border: [true, true, true, true]}         
        ],

        [{text:AsistenciaPDFMensualCifras._0_3_TOTAL_R,style: 'campos', colSpan: 2, border: [true, false, true, true]},
        {},
        {text:AsistenciaPDFMensualCifras._3_6_TOTAL_R,style: 'campos', colSpan: 2, border: [true, false, true, true]},
        {},
        {text:AsistenciaPDFMensualCifras._6_TOTAL_R,style: 'campos', colSpan: 2, border: [true, false, true, true]},
        {},
        {text:AsistenciaPDFMensualCifras._GESTANTES_R,style: 'campos', rowSpan: 3, border: [true, false, true, true]},
        {text:AsistenciaPDFMensualCifras._TOTAL_R,style: 'campos', rowSpan: 3, border: [true, false, true, true]}],

        [{text:'Niños de 0 a 3',style: 'titles2Real', border: [true, true, true, true]},
        {text:'Niñas de 0 a 3',style: 'titles2Real', border: [true, true, true, true]},
        {text:'Niños de 4 a 5',style: 'titles2Real', border: [true, true, true, true]},
        {text:'Niñas de 4 a 5',style: 'titles2Real', border: [true, true, true, true]},
        {text:'Niños 6',style: 'titles2Real', border: [true, true, true, true]},
        {text:'Niñas 6',style: 'titles2Real', border: [true, true, true, true]},
        {},{}],

        [{text:AsistenciaPDFMensualCifras._0_3_NINOS_R,style: 'campos',  border: [true, false, true, true]},              
        {text:AsistenciaPDFMensualCifras._0_3_NINAS_R,style: 'campos',  border: [true, false, true, true]},              
        {text:AsistenciaPDFMensualCifras._3_5_NINOS_R,style: 'campos', border: [true, false, true, true]},
        {text:AsistenciaPDFMensualCifras._3_5_NINAS_R,style: 'campos', border: [true, false, true, true]},
        {text:AsistenciaPDFMensualCifras._6_NINOS_R,style: 'campos', border: [true, false, true, true]},
        {text:AsistenciaPDFMensualCifras._6_NINAS_R,style: 'campos', border: [true, false, true, true]},
        {},{}],

        ]

      },layout: {
        vLineWidth: function (i, node) {
          return 0.5;
        },

        hLineWidth: function (i, node) {
          return 0.5;
        },
      }
    },

  ],


    styles: {
      header: {
        fontSize: 18,
        bold: true,
        margin: [0, 0, 0, 10]
      },

      subheader: {
        fontSize: 16,
        bold: true,
        alignment:'center',
        margin: [0, 15, 0, 0]
      },

      tableExample: {
        margin: [0, 5, 0, 15]
      },

      titles1:{
          bold: true,
          alignment:'center',
          fontSize: 9,          
          color: 'black'         
      },

      titles2Nuevos:{
          bold: true,
          alignment:'center',
          fontSize: 9,
          fillColor: '#FADBD8 ',        
          color: 'black'
      },

      titles2Real:{
        bold: true,
        alignment:'center',
        fontSize: 9,
        fillColor: '#D6EAF8',        
        color: 'black'
      },

      titles2Nino:{
        bold: true,
        alignment:'center',
        fontSize: 14,
        fillColor: '#7DCEA0',        
        color: 'black'
      },

      titles3Nino:{
        bold: true,
        alignment:'center',
        fontSize: 12,
        fillColor: '#7DCEA0',        
        color: 'black'
      },

      titles2Cifras:{
        bold: true,
        alignment:'center',
        fontSize: 14,
        fillColor: '#E59866',       
        color: 'black'
    }, 

    titles3Cifras:{
      bold: true,
      alignment:'center',
      fontSize: 12,
      fillColor: '#E59866',       
      color: 'black'
  }, 

  titles2Verde:{
    bold: true,
    alignment:'center',
    fontSize: 14,       
    color: 'green'
  },

  titles2Rojo:{
    bold: true,
    alignment:'center',
    fontSize: 14,       
    color: 'red'
  },

      campos:{
          alignment:'center',
          fontSize: 9,
          color: 'black'
      },

      titles:{
          bold: true,
          fontSize: 10,
          fillColor: '#eeeeff',
         color: 'black'
      },

      titlesA:{
        bold: true,
        alignment:'center',
        fontSize: 10,
        fillColor: '#eeeeff',
       color: 'black'
    },

      subtitles:{
          bold: true,
          margin: [0, 5, 0, 5],
          fontSize: 10,
         color: 'black'
      },

      tableHeader: {
        bold: true,
        fontSize: 13,
        color: 'black'

      }

    },

    images: {
      idartes_logo: 'data:image/png;base64,',
      nidos_logo: 'data:image/png;base64,'
    },

    defaultStyle: {
    }
  }
pdfMake.createPdf(dd).open();
}
});