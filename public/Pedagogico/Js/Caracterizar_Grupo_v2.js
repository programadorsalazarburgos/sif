...
$(function(){
	...
	var caracterizacion;
	var ultima_caracterizacion;
	var bandera_edicion = 0;
	var codigoGrupo ="";
	var id_caracterizacion = "";
	var tipo_grupo="";
	...
	$("#nombre_formador").html('FORMADOR: '+nombre_formador);
	$("#SL_grupo").html(parent.getGruposDeUnUsuario(parent.idUsuario)).selectpicker("refresh");
	$('#SL_grupo').on('change', function(e){
		$('#TXTA_DESCRIPCION_GRUPO').val('');
		$('#TXTA_CONVIVENCIA').val('');
		$('#TXTA_INTERESES').val(''); 
		$('#TXTA_PARTICULARIDADES').val('');
		$('#TXTA_ESPACIO').val('');
		$('#TXTA_OBSERVACIONES').val('');
		$("#SL_CICLO option[value='']").prop('selected', true);
		$("#SL_TIPO_CARACTERIZACION option[value='INICIAL']").prop('selected', true);
		bandera_edicion = 0;
		e.preventDefault();
		...
		cargarEncabezadoCaracterizacion(codigoGrupo, tipo_grupo, 'creacion', $(this).data('id-formador'));
		var datos = {
			p1: {
				'fk_grupo': codigoGrupo,
				'fk_id_linea_atencion': tipo_grupo,
				'fk_id_usuario_registro': session
			},
		}
		$.ajax({
			async : false,
			url: url_pedagogico_controller+'consultarUltimaCaracterizacionAprobada',
			type: 'POST',
			dataType: 'json',
			data: datos,
			...
			{
				if(retorno != null){
					ultima_caracterizacion = retorno;
	...
	$('#BT_CARGAR_DATOS').on('click', function(e){
		e.preventDefault();
		cargarContenidoCaracterizacion(ultima_caracterizacion, bandera_edicion);
		parent.swal("","La datos de la última caracterización aprobada fueron cargados","success");
	});
	...
	$("body").delegate("#input-finalizado-2", "change", function(){
		if ($('#input-finalizado-2').is(":checked")){
			$("#modalEditar .required").prop('required', true);
			$("#label-state-finalizado-2").text("Finalizado");
		}
		else{
			$("#label-state-finalizado-2").text("Sin Finalizar");
			$("#modalEditar .required").removeAttr('required');
		}
	});

	$("#table-listado-caracterizaciones").delegate(".editar", "click", function(){
		$("#modalEditar").modal('show');
		$("#FORM_CARACTERIZACION_EDITAR").html($("#FORM_CARACTERIZACION_CONTAINER").clone());
		$("#modalEditar #FORM_CARACTERIZACION_CONTAINER").append("<div class='col-lg-5 col-md-5 col-sm-5 text-right'><button type='button' class='btn btn-danger btn-lg' data-dismiss='modal'>Cerrar</button></div><div class='col-lg-2 col-md-2 col-sm-2 text-left'><div style='margin-top: 8px; margin-left: 50px;' class='material-switch pull-left' data-toggle='tooltip' title='Finalizado / Sin Finalizar' data-placement='bottom'><input  id='input-finalizado-2' name='input-finalizado-2'  class='checkState' type='checkbox'/><label for='input-finalizado-2' class='label-success'></label></div><label style='margin-top: 8px' class='col-lg-6 text-left' id='label-state-finalizado-2'>Sin Finalizar</label></div><div class='col-lg-5 col-md-5 col-sm-5 text-left'><button id='submit-editar-informe' class='btn btn-success' type='submit'>Guardar</button></div>");
		$('#modalEditar #TXTA_REVISION').summernote({
			minHeight: null,
			height: 100,             
			maxHeight: null,
			placeholder: 'Detalle su observación en este espacio.',
			lang: 'es-ES'
		});
		tipo_grupo = $(this).data('linea-atencion');
		cargarEncabezadoCaracterizacion($(this).data('id-grupo'), tipo_grupo, 'edicion', $(this).data('id-formador'));
		id_caracterizacion = $(this).data('id-caracterizacion');
		var datos = {
			p1: {
				'id_caracterizacion': id_caracterizacion
			}
		};
		$.ajax({
			async: false,
			url: url_pedagogico_controller+'getCaracterizacion',
			type:'POST',
			dataType: 'json',
			data: datos,
			success: function(datos_caracterizacion){
				if(datos_caracterizacion != null){
					caracterizacion = datos_caracterizacion;
					bandera_edicion = 1;
				}
			}
		});
		cargarContenidoCaracterizacion(caracterizacion, bandera_edicion);
		$("#modalEditar .campo").prop("readonly", false);
		$("#modalEditar .select").prop("disabled", false);
	});

	$("#table-listado-caracterizaciones").delegate(".ver", "click", function(){
		$("#modalEditar").modal('show');
		$("#FORM_CARACTERIZACION_EDITAR").html($("#FORM_CARACTERIZACION_CONTAINER").clone());
		$("#modalEditar #FORM_CARACTERIZACION_CONTAINER").append("<div class='col-lg-1 col-md-1 col-sm-1 col-lg-offset-11 col-md-offset-11 col-sm-offset-11 text-right'><button type='button' class='btn btn-danger btn-lg' data-dismiss='modal'>Cerrar</button></div>");
		tipo_grupo = $(this).data('linea-atencion');
		cargarEncabezadoCaracterizacion($(this).data('id-grupo'), tipo_grupo, 'edicion', $(this).data('id-formador'));
		id_caracterizacion = $(this).data('id-caracterizacion');
		var datos = {
	...
	$("#table-listado-caracterizaciones").delegate(".descargar", "click", function(){
		id_caracterizacion = $(this).data('id-caracterizacion');
		tipo_grupo = $(this).data('linea-atencion');
		construirEncabezadoPDF($(this).data('id-grupo'), tipo_grupo, id_caracterizacion, $(this).data('id-formador'));
	});

	function init() {
		tableListado = $('#table-listado-caracterizaciones').DataTable( {
			"paging":   true,
			"ordering": true,
			"info":     false,
			"columnDefs": [
			...
			buttons: [
			{
				...
				filename: 'ListadoCaracterizaciones',
				exportOptions: {
					columns: [ 1,2 ],
					modifier: {
						page: 'all'
	...
	function refreshListado() {
		...
		$.ajax({
			url: url_pedagogico_controller+"getListadoCaracterizaciones",
			type: 'POST',
			dataType: 'html',
			data: datos_ajax,
	...
	$("#FORM_CARACTERIZACION").on('submit', function(e){
		...
		if(!$('#SL_grupo').val()){
			parent.swal('Debes seleccionar un grupo','','warning');
		}
		else{
			var datos = {
				p1: {
					'fk_grupo': codigoGrupo,
					// 'vc_tipo': $("#SL_TIPO_CARACTERIZACION").val(),
					'fk_id_linea_atencion': tipo_grupo,
					'fk_id_usuario_registro': session
				},
			}
			$.ajax({
				async : false,
				url: url_pedagogico_controller+'validarExistenciaCaracterizacion',
				type: 'POST',
				dataType: 'html',
				data: datos,
				success: function(retorno)
				{
					if(retorno == 1){
						parent.swal("No se pudo guardar", "Existe una caracterización de este grupo en estado SIN FINALIZAR.<br>No puedes guardar nuevas caracterizaciones de este grupo mientras tenga caracterizaciones en progreso, debes trabajar sobre la que está en progreso hasta finalizarla, encuéntrala en la pestaña de LISTADO para ver/editar la existente.","error");
			...
			if(bandera_existencia == 0){
				...
				if($('#input-finalizado-2').is(":checked")){
					...
					texto = "La caracterización se guardará como borrador";
					estado = "guardada";
				}
				parent.swal({
					title: titulo,
					...
					var datos = {
						p1: {
							'fk_grupo': codigoGrupo,
							'fk_ciclo': $("#SL_CICLO").val(),
							'vc_tipo': $("#SL_TIPO_CARACTERIZACION").val(),
							'fk_id_linea_atencion': tipo_grupo,
							'tx_particularidades': $("#TXTA_PARTICULARIDADES").val(),
							'tx_descripcion_grupo': $("#TXTA_DESCRIPCION_GRUPO").val(),
							'tx_descripcion_intereses': $("#TXTA_INTERESES").val(),
							'tx_descripcion_espacio': $("#TXTA_ESPACIO").val(),
							'tx_convivencia': $("#TXTA_CONVIVENCIA").val(),
							'tx_observaciones': $("#TXTA_OBSERVACIONES").val(),
							'fk_id_usuario_registro': session,
							'in_version': 2,
					...
					$.ajax({
						async : false,
						url: url_pedagogico_controller+'guardarCaracterizacion',
						type: 'POST',
						dataType: 'html',
						data: datos,
						success: function(retorno)
						{
							if(retorno == 1){
								parent.swal("Guardado", "La caracterización ha sido "+estado,"success");
								refreshListado();
								$("#a_listado").click();
							}
							else{
								parent.swal("Algo salió mal", "No se pudo guardar la caracterizacion","warning");  
							}
						}
					});
				}, function(dismiss){
	...
	$("#FORM_CARACTERIZACION_EDITAR").on('submit', function(e){
		e.preventDefault();
		var finalizado;
		var estado;
		var texto = '';
		...
		parent.swal({
			...
			var datos = {
				p1: {
					'pk_id_caracterizacion': id_caracterizacion,
					'fk_ciclo': $("#modalEditar #SL_CICLO").val(),
					'vc_tipo': $("#modalEditar #SL_TIPO_CARACTERIZACION").val(),
					'fk_id_linea_atencion': tipo_grupo,
					'tx_particularidades': $("#modalEditar #TXTA_PARTICULARIDADES").val(),
					'tx_descripcion_grupo': $("#modalEditar #TXTA_DESCRIPCION_GRUPO").val(),
					'tx_descripcion_intereses': $("#modalEditar #TXTA_INTERESES").val(),
					'tx_descripcion_espacio': $("#modalEditar #TXTA_ESPACIO").val(),
					'tx_convivencia': $("#modalEditar #TXTA_CONVIVENCIA").val(),
					'tx_observaciones': $("#modalEditar #TXTA_OBSERVACIONES").val(),
					'fk_id_usuario_registro': session,
					'in_version': 2,
					'in_finalizado': finalizado,
					'in_estado': null
			...
			$.ajax({
				...
				{
					if(retorno == 1){
						parent.swal("Realizado", "La caracterización ha sido "+estado,"success");
						$("#modalEditar").modal('hide');
						refreshListado();
					}
					else{
						parent.swal("Algo salió mal", "No se pudo realizar la acción","warning");  
					}
				}
			});
		}, function(dismiss){
...
function cargarContenidoCaracterizacion(caracterizacion, bandera_edicion) {
	$('#TXTA_DESCRIPCION_GRUPO').val('');
	$('#TXTA_CONVIVENCIA').val('');
	$('#TXTA_INTERESES').val(''); 
	$('#TXTA_PARTICULARIDADES').val('');
	...
	$('#modalEditar #TXTA_DESCRIPCION_GRUPO').val('');
	$('#modalEditar #TXTA_CONVIVENCIA').val('');
	$('#modalEditar #TXTA_INTERESES').val(''); 
	$('#modalEditar #TXTA_PARTICULARIDADES').val('');
	$('#modalEditar #TXTA_ESPACIO').val('');
	$('#modalEditar #TXTA_OBSERVACIONES').val('');
	if (bandera_edicion == 1) {
		$("#modalEditar #SL_CICLO option[value='" + caracterizacion.FK_Ciclo + "']").prop('selected', true);
		$("#modalEditar #SL_TIPO_CARACTERIZACION option[value='" + caracterizacion.VC_Tipo + "']").prop('selected', true);
		$('#modalEditar #TXTA_DESCRIPCION_GRUPO').val(caracterizacion.TX_Descripcion_Grupo);
		$('#modalEditar #TXTA_CONVIVENCIA').val(caracterizacion.TX_Convivencia);
		$('#modalEditar #TXTA_INTERESES').val(caracterizacion.TX_Descripcion_Intereses); 
		$('#modalEditar #TXTA_PARTICULARIDADES').val(caracterizacion.TX_Particularidades);
		$('#modalEditar #TXTA_ESPACIO').val(caracterizacion.TX_Descripcion_Espacio);
		$('#modalEditar #TXTA_OBSERVACIONES').val(caracterizacion.TX_Observaciones);
		if(caracterizacion.TX_Observacion != null){
			$('#modalEditar #div_correcciones').removeAttr('hidden');
			$('#modalEditar #TXTA_REVISION').summernote('code',caracterizacion.TX_Observacion);
		...
		$("#SL_CICLO option[value='" + caracterizacion.FK_Ciclo + "']").prop('selected', true).selectpicker('refresh');
		$("#SL_TIPO_CARACTERIZACION option[value='" + caracterizacion.VC_Tipo + "']").prop('selected', true).selectpicker('refresh');
...
function cargarEncabezadoCaracterizacion(codigoGrupo, tipo_grupo, trigger, idformador){
	var datos = { 
		p1: {
	...
	var id_modal = '';
	if(trigger == 'edicion') { id_modal = "#modalEditar"; }
	$.ajax({
		async : false,
		...
		success: function(INFO)
		{
			if(!INFO[0]){
				...
				if(tipo_grupo == 'arte_escuela'){
					var id_grupo = "AE-"+codigoGrupo;
					$(id_modal+" #id_del_grupo").html("CÓDIGO DEL GRUPO: "+id_grupo);
					$(id_modal+" #LB_Area_Artistica").html("ÁREA ARTÍSTICA: "+INFO[0].VC_Nom_Area);
					$(id_modal+" #LB_Linea_Atencion").html('LÍNEA DE ATENCIÓN: ARTE EN LA ESCUELA');
					$(id_modal+" #LB_Clan").html(INFO[0].VC_Nom_Clan);
					$(id_modal+" #LB_Fecha_Inicio").html("FECHA DE INICIO FORMACIÓN GRUPO: El grupo no ha iniciado");
					var lugar_atencion = "";
					if(INFO[0].IN_lugar_atencion == 1){
						lugar_atencion = "(COLEGIO) - "+INFO[0].VC_Nom_Colegio;
					}else{
						if(INFO[0].IN_lugar_atencion == 2){
							lugar_atencion = "(CREA) - "+INFO[0].VC_Nom_Colegio;        
						}
						else{
							if(INFO[0].IN_lugar_atencion == 3) {
								lugar_atencion = "(CREA Y COLEGIO) - "+INFO[0].VC_Nom_Colegio;              
							}
						}
					}
					$(id_modal+" #LB_Lugar_Atencion").html("LUGAR DE ATENCIÓN: "+lugar_atencion);
				}
				if(tipo_grupo == 'emprende_clan'){
					var id_grupo = "EC-"+codigoGrupo;
					$(id_modal+" #id_del_grupo").html("CÓDIGO DEL GRUPO: "+id_grupo);
					$(id_modal+" #LB_Area_Artistica").html("ÁREA ARTÍSTICA: "+INFO[0].VC_Nom_Area);
					$(id_modal+" #LB_Linea_Atencion").html('LÍNEA DE ATENCIÓN: EMPRENDE CREA');
					$(id_modal+" #LB_Clan").html(INFO[0].VC_Nom_Clan);
					$(id_modal+" #LB_Fecha_Inicio").html("FECHA DE INICIO FORMACIÓN GRUPO: El grupo no ha iniciado");
				...
				if(tipo_grupo == 'laboratorio_clan'){
					var id_grupo = "LC-"+codigoGrupo;
					$(id_modal+" #id_del_grupo").html("CÓDIGO DEL GRUPO: "+id_grupo); 
					$(id_modal+" #LB_Area_Artistica").html("ÁREA ARTÍSTICA: "+INFO[0].VC_Nom_Area);
					$(id_modal+" #LB_Linea_Atencion").html('LÍNEA DE ATENCIÓN: LABORATORIO CREA');
					$(id_modal+" #LB_Clan").html(INFO[0].VC_Nom_Clan);
					$(id_modal+" #LB_Fecha_Inicio").html("FECHA DE INICIO FORMACIÓN GRUPO: El grupo no ha iniciado");
					var lugar_atencion = "";
					if(INFO[0].IN_lugar_atencion == 1){
						...
						if(INFO[0].IN_lugar_atencion == 2){
							lugar_atencion = "(ALIADO) - "+INFO[0].TX_Nombre_Aliado;
						}
						else{
							if(INFO[0].IN_lugar_atencion == 3) {
								lugar_atencion = "(ESPACIO ALTERNO) - "+INFO[0].TX_Sitio;
							}
						}
					}
					$(id_modal+" #LB_Lugar_Atencion").html("LUGAR DE ATENCIÓN: "+lugar_atencion);
	...
	$(id_modal+" #LB_Horario").html("HORARIO:<br>"+consultarHorario(datos));
	$(id_modal+" #LB_Fecha_Inicio").html("FECHA DE INICIO FORMACIÓN GRUPO: "+consultarFechaPrimerClase(datos));
	$(id_modal+" #LB_Promedio_Asistentes").html("PROMEDIO ASISTENTES: "+consultarPromedioAsistentes(datos)+" ASISTENTES POR SESIÓN");
}

function construirEncabezadoPDF(codigoGrupo, tipo_grupo, id_caracterizacion, idformador){
	var contenido_caracterizacion = "";
	var ciclo_grupo = "";
	var tipo_caracterizacion = "";
	var datos = {
		p1: {
			'id_caracterizacion': id_caracterizacion
		}
	...
	$.ajax({
		...
		success: function(datos_caracterizacion){
			if(datos_caracterizacion != null){
				contenido_caracterizacion = datos_caracterizacion;
				ciclo_grupo = contenido_caracterizacion.FK_Ciclo;
				tipo_caracterizacion = contenido_caracterizacion.VC_Tipo;
			}
		}
	});
	...
	var grupo = ""; var area_artistica = ""; var linea_atencion = ""; var crea = ""; var lugar_atencion = ""; var horario_grupo = ""; var primer_clase_grupo = ""; var promedio_asistencia_grupo = "Sin asistencia";
	$.ajax({
		async : false,
		url: url_pedagogico_controller+'consultarInformacionGrupo',
		type: 'POST',
		...
		success: function(INFO)
		{
			if(!INFO[0]){
				...
				area_artistica = INFO[0].VC_Nom_Area;
				crea = INFO[0].VC_Nom_Clan;
				primer_clase_grupo = "El grupo no ha iniciado";
				if(tipo_grupo == 'arte_escuela'){
					grupo = "AE-"+codigoGrupo;
				...
				if(tipo_grupo == 'emprende_clan'){
					grupo = "EC-"+codigoGrupo;
					linea_atencion = "EMPRENDE CREA";
					ciclo_grupo = "NO APLICA"
					lugar_atencion = "CREA";
				}
				if(tipo_grupo == 'laboratorio_clan'){
					grupo = "LC-"+codigoGrupo;
					ciclo_grupo = "NO APLICA";
	...
	horario_grupo = consultarHorario(datos);
	primer_clase_grupo = consultarFechaPrimerClase(datos);
	promedio_asistencia_grupo = consultarPromedioAsistentes(datos)+" ASISTENTES POR TALLER";
	...
	encabezado_pdf.push(
	{
		...
		table: {
			widths: [70, 180, 100, '*'],
			body: [
			[{text:'Formador',style: 'subtitle',fillColor:'#e6e6e6',border: [true, true, true, false]},{text:nombre_formador,style: 'generalcenter',border: [true, true, true, false]},{text:'Área Artística',style: 'subtitle',fillColor:'#e6e6e6',border: [true, true, true, false]}, {text:area_artistica,style: 'generalcenter',border: [true, true, true, false]}],
			[{text:'Línea de Atención',style: 'subtitle',fillColor:'#e6e6e6',border: [true, true, true, false]},{text:linea_atencion,style: 'generalcenter',border: [true, true, true, false]},{text:'CREA',style: 'subtitle',fillColor:'#e6e6e6',border: [true, true, true, false]}, {text:crea,style: 'generalcenter',border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: [259,100,75, '*'],
			body: [
			[{text:'Fecha de inicio formación grupo',style: 'subtitlecenter',fillColor:'#e6e6e6',border: [true, true, true, false]},{text:primer_clase_grupo,style: 'generalcenter',border: [true, true, true, false]},{text:'Código del Grupo',style: 'subtitle',fillColor:'#e6e6e6',border: [true, true, true, false]}, {text:grupo,style: 'generalcenter',border: [true, true, true, false]}],
			]
		},layout: {
			vLineWidth: function (i, node) {
				return 0.9;
	...
	},
	{
		...
		table: {
			widths: [259,'*'],
			body: [
			[{text:'Lugar de atención (Localidad- Entidad-IED- Crea)',style: 'subtitlecenter',fillColor:'#e6e6e6',border: [true, true, true, false]},{text:lugar_atencion,style: 'generalcenter',border: [true, true, true, false]}]
			]
		},layout: {
			vLineWidth: function (i, node) {
				return 0.9;
	...
	},
	{
		...
		table: {
			widths: [70, 180, 100, '*'],
			body: [
			[{text:'Ciclo AE',style: 'subtitle',fillColor:'#e6e6e6',border: [true, true, true, false]},{text:ciclo_grupo,style: 'subtitle',border: [true, true, true, false]},{text:'Tipo de caracterización',style: 'subtitle',fillColor:'#e6e6e6',border: [true, true, true, false]}, {text:tipo_caracterizacion,style: 'generalcenter',border: [true, true, true, false]}],
			]
		},layout: {
			vLineWidth: function (i, node) {
				return 0.9;
	...
	},
	{
		...
		table: {
			widths: [70, 180, 180, '*'],
			body: [
			[{text:'Horario',style: 'subtitle',fillColor:'#e6e6e6',border: [true, true, true, false]},{text:horario_grupo.replace(/<\/?[^>]+(>|$)/g, ""),style: 'generalcenter',border: [true, true, true, false]},{text:'Promedio de asistentes',style: 'subtitlecenter',fillColor:'#e6e6e6',border: [true, true, true, false]}, {text:promedio_asistencia_grupo,style: 'generalcenter',border: [true, true, true, false]}],
			]
		},layout: {
			vLineWidth: function (i, node) {
				return 0.9;
	...
	var contenidoPDF = construirContenidoPDF(contenido_caracterizacion);
	descargarPDFCaracterizacion(encabezado_pdf, contenidoPDF);
}

function construirContenidoPDF(contenido_caracterizacion) {
	var contenidoPDF = [];
	contenidoPDF.push(
	{
		style: 'tableExample',
		table: {
			widths: ['*'],
			body: [
			[{text:'1. Análisis del contexto.',style: 'titles', border: [true, true, true, false]}],
			[{text:'(Descripción de condiciones, situaciones o experiencias de vida significativas de los participantes que pueden afectar los procesos, es decir, aquellas cosas que no podemos obviar. Es importante tener en cuenta tanto aspectos positivos como negativos.)', style: 'subtitle', color:'#595959', border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:contenido_caracterizacion.TX_Particularidades,style: 'general', border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:'2. Caracterización.',style: 'titles', border: [true, true, true, false]}],
			[{text:'2.1 Descripción general del grupo:\nPara La Línea de Arte en la Escuela: Breve reseña del grupo que contenga los siguientes datos: Edades de los participantes, Ciclo de formación al que pertenecen o si son multigrados, grupos de aceleración, enuncie los grados.Gustos específicos, expectativas frente al centro de interés y los talleres según el área artística, que incluya territorio, nivel socioeconómico, colegio, experiencias previas de formación, o si no han tenido ninguna experiencia previa.\nPara la Línea de Emprende Crea: Breve reseña del grupo que contenga los siguientes datos: Edades de los participantes y gustos específicos, expectativas frente al proyecto artístico, que incluya territorio, nivel socioeconómico, experiencias previas de formación, o si no han tenido ninguna experiencia previa.\nPara la Línea de Laboratorio Crea: Breve reseña del grupo que contenga los siguientes datos: Edades de los participantes, comunidades, poblaciones específicas y gustos específicos, expectativas frente al proceso orientado por el área y/o la entidad. Territorio, nivel socioeconómico, condición social, experiencias con lo artístico, o si no han tenido ninguna experiencia previa.', style: 'subtitle', color:'#595959', border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:contenido_caracterizacion.TX_Descripcion_Grupo,style: 'general', border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:'2.2. Identificación de intereses del grupo:\nPara la Línea de Arte en la Escuela: Tiene como objetivo dar cuenta de manera descriptiva los intereses de los niños, niñas y adolescentes, relacionados con “Explorar, ir más allá́, expandir posibilidades: se aprende a ser más exigente consigo mismo, a ser creativo, a intentar otras formas de trabajar y de pensar, de explorar y aprender de errores y accidentes.” (Idartes, 2016, 77).\nPara la Línea de Emprende Crea: Identificación de un tema (s), pregunta (s) de los integrantes del grupo, para la construcción de un proyecto artístico.\nPara la Línea de Laboratorio Crea: Identificación de un tema (s), pregunta (s) de los integrantes del grupo, para la construcción del proceso artístico con diversa naturaleza, disciplinar, interdisciplinar o transdisciplinar.', style: 'subtitle', color:'#595959', border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:contenido_caracterizacion.TX_Descripcion_Intereses,style: 'general', border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:'2.3 Descripción del espacio: El espacio en el que se ofrece la formación cuenta con mesas para el desarrollo de las actividades, la luz es favorable, sin embargo en relación a la cantidad de participantes ameritaría pensar en otra ubicación. Tiene aislamiento acústico y ventilación adecuada.', style: 'subtitle', color:'#595959', border: [true, true, true, false]}]
			]
		},layout: {
			vLineWidth: function (i, node) {
				return 0.9;
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:contenido_caracterizacion.TX_Descripcion_Espacio,style: 'general', border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:'3. Aspectos de Conveniencia.',style: 'titles', border: [true, true, true, false]}],
			[{text:'Identificar los aspectos de convivencia del grupo, dinámicas de relación, comunicación asertiva o no, concertaciones y acuerdos de aula.\nPara la Línea de Arte en la Escuela se refiere al componente socio-afectivo que corresponde a las orientaciones de la Secretaría de Educación. (Dimensión afectiva, corporal y relacional. Describir si es notoria o no la capacidad perceptiva, reflexiva, cooperativa, sentido de colectividad, integración).\nPara la Línea de Emprende y laboratorio Crea se refiere a la Dimensión afectiva, corporal y relacional. Describir si es notoria o no la capacidad perceptiva, reflexiva, cooperativa, sentido de colectividad, integración.', style: 'subtitle', color:'#595959', border: [true, true, true, false]}]
			]
		},layout: {
			vLineWidth: function (i, node) {
				return 0.9;
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:contenido_caracterizacion.TX_Convivencia,style: 'general', border: [true, true, true, false]}]
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:'4. Observaciones / Recomendaciones.',style: 'titles', border: [true, true, true, false]}],
			]
		},layout: {
			vLineWidth: function (i, node) {
				return 0.9;
	...
	},
	{
		...
		table: {
			widths: ['*'],
			body: [
			[{text:contenido_caracterizacion.TX_Observaciones,style: 'general', border: [true, true, true, true]}]
...
function consultarHorario(datos){
	var resultado = ""; 
	$.ajax({
		async : false,
...
function consultarFechaPrimerClase(datos){
	var resultado = "";
	$.ajax({
		async : false,
		url: url_pedagogico_controller+'consultarFechaPrimerClaseGrupo',
		type: 'POST',
		dataType: 'json',
		data: datos,
		success: function(fecha_inicio)
		{
			if(!fecha_inicio){
			}
			else{
				resultado = fecha_inicio[0].primer_clase;
			}
		}
	});
	return resultado;
...
function consultarPromedioAsistentes(datos){
	var resultado = "";
	$.ajax({
		async : false,
		url: url_pedagogico_controller+'consultarPromedioAsistentesGrupo',
		type: 'POST',
		dataType: 'json',
		data: datos,
		success: function(promedio_asistentes)
		{
			if(!promedio_asistentes){
			}
			else{
				resultado = promedio_asistentes[0].PROMEDIO;
			}
		}
	});
	return resultado;
...
function descargarPDFCaracterizacion(encabezado_pdf, contenido_pdf){
	var count = 0 ;
	var bandera = false;
	var dd = {
		pageSize: 'Legal',
		...
		content: [
		{
			...
			table: {
				...
				body: [
				[
				{image: 'idartes_logo',width: 80,rowSpan: 4,alignment:'center', margin:[0,2,0,0]},
				{text:'GESTIÓN DE FORMACIÓN EN LAS PRÁCTICAS ARTÍSTICAS',style: 'titles',rowSpan: 2,margin: [0, 11, 0, 0]},
				{text:'Código: 3TR-GFI-F-01',style: 'general',margin: [0, 2, 0, 0]}
				],
				[
				{},
				{text:{}},
				{text:'Fecha: 01/02/2019',style: 'general',margin: [0, 2, 0, 0]}
				],
				[
				{},
				{text:'FICHA DE CARACTERIZACIÓN \n PROCESO DE FORMACIÓN ARTÍSTICA \n AE / EC / LC',style: 'titles',rowSpan: 2,margin: [0, 2, 0, 0]},
	...
	pdfMake.createPdf(dd).download("caracterizaciónPDF.pdf");
